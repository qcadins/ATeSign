...
public class DataVerif {
	...
	getPengaturanTenant(Connection conn, String emailLogin) {
		...
		resultSet = stm.executeQuery("select mst.usr_upd,TO_CHAR(mst.dtm_upd,'yyyy-MM-dd'), mst.ref_number_label, mst.upload_url, mst.threshold_balance,mst.email_reminder_dest, mst.automatic_stamping_after_sign, mst.activation_callback_url from ms_tenant mst join ms_useroftenant muot on mst.id_ms_tenant = muot.id_ms_tenant join am_msuser amm on muot.id_ms_user = amm.id_ms_user where amm.login_id = '"+emailLogin+"'")
		metadata = resultSet.metaData
