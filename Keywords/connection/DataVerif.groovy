...
class DataVerif {
	...
	getSaldo(Connection conn, String user) {
		...
		resultSet = stm.executeQuery("SELECT qty FROM tr_balance_mutation where usr_crt = '" + maskingEsign.maskData(user) + "' ORDER BY trx_date DESC LIMIT 1")
	...
	@Keyword
	getSaldoTrx(Connection conn, String desc) {
		stm = conn.createStatement()
	...
	getCountTrx(Connection conn, String email, String notelp, String desc) {
		...
		resultSet = stm.executeQuery("SELECT COUNT(*) FROM tr_balance_mutation tbm JOIN ms_lov ml ON ml.id_lov = tbm.lov_trx_type JOIN am_msuser amu ON amu.id_ms_user = tbm.id_ms_user WHERE description = '" + desc + "' AND (tbm.usr_crt = '" + maskingEsign.maskData(email) + "' OR tbm. = '" + maskingEsign.maskData(notelp) + "')")
	...
	getBusinessLineOfficeCode(Connection conn, String value, String type) {
		...
		if (type.equalsIgnoreCase('Register')) {
			...
			resultSet = stm.executeQuery("SELECT '-1', COALESCE(tdh.id_ms_business_line) AS id_ms_business_line, COALESCE(tdh.id_ms_office) AS id_ms_office FROM am_msuser amu LEFT JOIN tr_document_d_sign tds ON tds.id_ms_user = amu.id_ms_user LEFT JOIN tr_document_d td on tds.id_document_d = td.id_document_d LEFT JOIN tr_document_h tdh ON td.id_document_h = tdh.id_document_h  WHERE amu.login_id = '" + value.toUpperCase() + "' OR amu.hashed_phone = encode(sha256('" + value.toUpperCase() + "'), 'hex') ORDER BY td.id_document_d DESC LIMIT 1")
