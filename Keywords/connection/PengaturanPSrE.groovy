...
public class PengaturanPSrE {
	...
	getPsrePriority(Connection conn) {
		...
		resultSet = stm.executeQuery("SELECT VENDOR_NAME FROM ms_vendoroftenant mvt JOIN ms_tenant mt ON mvt.id_ms_tenant = mt.id_ms_tenant JOIN ms_vendor mv ON mvt.id_ms_vendor = mv.id_ms_vendor join ms_lov ml on ml.id_lov = mv.lov_vendor_type WHERE mt.tenant_code = '"+ GlobalVariable.Tenant +"' AND ml.description = 'PSRE' AND is_operating = '1' AND mv.is_active = '1' ORDER BY mvt.default_vendor ASC")
