...
class JobResult {
	...
	jobResultDB(Connection conn, String startDate, String endDate) {
		...
		resultSet = stm.executeQuery("select tjr.usr_crt, msj.job_name, to_char(tjr.dtm_crt,'dd-Mon-yyyy hh24:mi:ss'), to_char(tjr.process_start_time,'dd-Mon-yyyy hh24:mi:ss'), to_char(tjr.process_finish_time,'dd-Mon-yyyy hh24:mi:ss'), cast(round(EXTRACT(EPOCH FROM (tjr.process_finish_time - tjr.process_start_time))) as varchar) AS duration_second, case when tjr.process_status = 0 then 'New' WHEN tjr.process_status = 1 then 'In Progress' when tjr.process_status = 2 then 'Completed' when tjr.process_status = 3 then 'Cancelled' when tjr.process_status = 4 then 'Failed' when tjr.process_status = 5 then 'Deleted' end from tr_job_result tjr left join ms_lov msl on msl.id_lov = tjr.lov_job_type left join ms_job msj on msj.id_ms_job = tjr.id_ms_job where tjr.process_start_time BETWEEN '" + startDate + "' AND '" + endDate + "'order by tjr.dtm_crt desc")
