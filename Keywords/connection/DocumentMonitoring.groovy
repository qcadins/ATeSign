...
class DocumentMonitoring {
	...
	getInputDocumentMonitoring(Connection conn, String refNumber) {
		...
		resultSet = stm.executeQuery("select case when amm.full_name != '' or amm.full_name != null then amm.full_name else '' end, TO_CHAR(tdd.request_date, 'yyyy-mm-dd'), case when tdd.completed_date is not null then TO_CHAR(tdd.completed_date,'yyyy-mm-dd') else '' end, msl_doctype.description, msl.description,CASE WHEN msr.region_name != '' THEN msr.region_name else 'All' END, CASE WHEN mso.office_name != '' THEN mso.office_name else 'All' END, CASE WHEN tdh.proses_materai = 0 THEN 'Not Started' WHEN tdh.proses_materai IN (1, 51, 321, 521, 61) THEN 'Failed' WHEN tdh.proses_materai IN (2, 52, 322, 522, 5, 55, 325, 525, 65, 62) THEN 'In Progress' WHEN tdh.proses_materai IN (3, 53, 323, 523, 63) THEN 'Success' END, case when tdh.is_active = '1' THEN 'Active' else 'Inactive' END from tr_document_h tdh left join ms_office mso on tdh.id_ms_office = mso.id_ms_office left join ms_region msr on mso.id_ms_region = msr.id_ms_region join tr_document_d tdd on tdd.id_document_h = tdh.id_document_h left join ms_lov msl on tdd.lov_sign_status = msl.id_lov left join am_msuser amm on tdh.id_msuser_customer = amm.id_ms_user left join ms_lov msl_doctype on tdh.lov_doc_type = msl_doctype.id_lov left join tr_document_d_stampduty tddstampduty on tdd.id_document_d = tddstampduty.id_document_d where tdh.ref_number = '" + refNumber + "'  order by tdh.dtm_crt desc limit 1")
		metadata = resultSet.metaData
	...
	getDocumentMonitoringBasedOnEmbed(Connection conn, String refNumber) {
		...
		resultSet = stm.executeQuery("SELECT tdh.ref_number, msl_doctype.description, CASE WHEN tdd.id_ms_doc_template IS NULL THEN CASE WHEN tdd.document_name IS NULL THEN '' ELSE tdd.document_name END ELSE mdt.doc_template_name END, CASE WHEN amm.full_name IS NULL THEN '' ELSE amm.full_name END, TO_CHAR(tdd.request_date, 'DD-Mon-YYYY HH24:MI'), CASE WHEN tdd.completed_date IS NULL THEN '-' ELSE TO_CHAR(tdd.completed_date, 'DD-Mon-YYYY HH24:MI') END, msl_signstatus.description, tdh.is_active, mso.office_name, CASE WHEN msr.region_name != null or msr.region_name IS NOT NULL or msr.region_name != '' THEN msr.region_name else '' end, CASE WHEN tdh.proses_materai = 0 THEN 'Not Started' WHEN tdh.proses_materai IN (1, 51, 321, 521, 61) THEN 'Failed' WHEN tdh.proses_materai IN (2, 52, 322, 522, 5, 55, 325, 525, 65, 62) THEN 'In Progress' WHEN tdh.proses_materai IN (3, 53, 323, 523, 63) THEN 'Success' END FROM tr_document_d tdd JOIN tr_document_h tdh ON tdd.id_document_h = tdh.id_document_h LEFT JOIN ms_lov msl_doctype ON tdh.lov_doc_type = msl_doctype.id_lov LEFT JOIN ms_doc_template mdt ON tdd.id_ms_doc_template = mdt.id_doc_template LEFT JOIN am_msuser amm ON tdh.id_msuser_customer = amm.id_ms_user LEFT JOIN ms_lov msl_signstatus ON tdd.lov_sign_status = msl_signstatus.id_lov LEFT JOIN ms_office mso ON tdh.id_ms_office = mso.id_ms_office LEFT JOIN ms_region msr ON mso.id_ms_region = msr.id_ms_region LEFT JOIN ms_tenant mst on tdh.id_ms_tenant = mst.id_ms_tenant WHERE tdh.ref_number = '" + refNumber + "' AND mst.tenant_code = '" + GlobalVariable.Tenant + "' ORDER BY tdd.id_document_d asc")
