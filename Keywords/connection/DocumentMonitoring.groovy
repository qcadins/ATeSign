...
public class DocumentMonitoring {
	...
	getInputDocumentMonitoring(Connection conn, String refNumber) {
		...
		resultSet = stm.executeQuery("select case when amm.full_name != '' or amm.full_name != null then amm.full_name else '' end, TO_CHAR(tdd.request_date, 'yyyy-mm-dd'), case when tdd.completed_date is not null then TO_CHAR(tdd.completed_date,'yyyy-mm-dd') else '' end, msl_doctype.description, msl.description, msr.region_name, mso.office_name from tr_document_h tdh left join ms_office mso on tdh.id_ms_office = mso.id_ms_office left join ms_region msr on mso.id_ms_region = msr.id_ms_region join tr_document_d tdd on tdd.id_document_h = tdh.id_document_h left join ms_lov msl on tdd.lov_sign_status = msl.id_lov left join am_msuser amm on tdh.id_msuser_customer = amm.id_ms_user left join ms_lov msl_doctype on tdh.lov_doc_type = msl_doctype.id_lov left join tr_document_d_stampduty tddstampduty on tdd.id_document_d = tddstampduty.id_document_d where tdh.ref_number = '"+refNumber+"'  order by tdh.dtm_crt desc limit 1")
		metadata = resultSet.metaData
	...
	getTotalStampingandTotalMaterai(Connection conn, String refNumber) {
		...
		resultSet = stm.executeQuery("select tdd.total_stamping, tdd.total_materai from tr_document_d tdd join tr_document_h tdh on tdd.id_document_h = tdh.id_document_h where tdh.ref_number = '" + refNumber + "' ORDER BY tdh.dtm_crt desc")
		metadata = resultSet.metaData
