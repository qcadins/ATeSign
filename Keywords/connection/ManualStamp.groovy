...
public class ManualStamp {
	...
	getInputeMeteraiMonitoring(Connection conn, String refNumber, String tenantCode) {
		...
		resultSet = stm.executeQuery("select tdh.ref_number,case when mpdt.doc_name != '' or mpdt.doc_name != null then mpdt.doc_name else '' end, msl1.description,CASE WHEN mdt.doc_template_description IS NULL THEN 'MANUAL' ELSE mdt.doc_template_description END, TO_CHAR(tddstamp.start_stamp_process, 'yyyy-mm-dd'), case when tddstamp.stamping_date is not null or tddstamp.stamping_date != null then TO_CHAR(tddstamp.stamping_date, 'yyyy-mm-dd') else '' END, CASE WHEN tdh.proses_materai = 0 THEN 'Not Started' WHEN tdh.proses_materai IN (1, 51, 321, 521) THEN 'Failed' WHEN tdh.proses_materai IN (2, 52, 322, 522, 5, 55, 325, 525) THEN 'In Progress' WHEN tdh.proses_materai IN (3, 53, 323, 523) THEN 'Success' END  ,case when tsd.stamp_duty_no is not null then tsd.stamp_duty_no else '' end,mso.office_name , case when tdh.is_postpaid_stampduty = '1' then 'Pemungut' else 'Non Pemungut' end  from tr_document_h tdh left join tr_document_d tdd on tdd.id_Document_h = tdh.id_document_h left join ms_lov msl on tdd.lov_payment_sign_type = msl.id_lov left join ms_lov msl1 on tdh.lov_doc_type = msl1.id_lov left join ms_peruri_doc_type mpdt on tdd.id_peruri_doc_type = mpdt.id_peruri_doc_type left join ms_doc_template mdt on tdd.id_ms_doc_template = mdt.id_doc_template left join ms_vendor msv on tdd.id_ms_vendor = msv.id_ms_vendor left join ms_tenant mst on tdh.id_ms_tenant = mst.id_ms_tenant left join ms_office mso on tdh.id_ms_office = mso.id_ms_office left join tr_document_d_stampduty tddstamp on tdd.id_document_d = tddstamp.id_document_d LEFT join tr_stamp_duty tsd on tddstamp.id_stamp_duty = tsd.id_stamp_duty where tdh.ref_number = '"+refNumber+"' and mst.tenant_code = '"+tenantCode+"'")
