...
class ManualSign {
	...
	getVendorofTenant(Connection conn, String tenantCode) {
		...
		resultSet = stm.executeQuery("select msv.vendor_name from ms_vendoroftenant mvot left join ms_tenant mst on mvot.id_ms_tenant = mst.id_ms_tenant left join ms_vendor msv on mvot.id_ms_vendor = msv.id_ms_vendor where mst.tenant_code = '" + tenantCode + "' and msv.is_active = '1' and msv.is_operating = '1' and mvot.default_vendor > '0' order by mvot.id_ms_vendoroftenant asc")
	...
	getManualSign(Connection conn, String refNumber) {
		...
		resultSet = stm.executeQuery("select msv.vendor_name, tdh.ref_Number, tdd.document_name, TO_CHAR(tdd.request_date, 'yyyy-mm-dd'), msl.description ,tdh.automatic_stamping_after_sign, case when mpdt.doc_name != '' or mpdt.doc_name != null then mpdt.doc_name else '' end, tdh.total_document, tdh.is_manual_upload, CASE WHEN tdd.is_sequence != '0' or tdd.is_sequence != null THEN 'Ya' else 'Tidak' end, mso.office_code, msr.region_code, mbl.business_line_code, tdd.use_sign_qr, amm.login_id from tr_document_h tdh left join tr_document_d tdd on tdd.id_Document_h = tdh.id_document_h left join ms_lov msl on tdd.lov_payment_sign_type = msl.id_lov left join ms_peruri_doc_type mpdt on tdd.id_peruri_doc_type = mpdt.id_peruri_doc_type left join ms_vendor msv on tdd.id_ms_vendor = msv.id_ms_vendor left join ms_office mso on tdh.id_ms_office = mso.id_ms_office left join ms_region msr on mso.id_ms_region = msr.id_ms_region left join ms_business_line mbl on tdh.id_ms_business_line = mbl.id_ms_business_line left join am_msuser amm on amm.id_ms_user = tdh.id_msuser_request_by where tdh.ref_number = '" + refNumber + "'")
	...
	@Keyword
	getAPIManualSign(Connection conn, String refNumber) {
		stm = conn.createStatement()

		resultSet = stm.executeQuery("select msv.vendor_name, tdh.ref_Number, tdd.document_name, TO_CHAR(tdd.request_date, 'yyyy-mm-dd'), mpdt.peruri_doc_id, tdh.automatic_stamping_after_sign, msl.code ,tdd.is_sequence, mso.office_code, msr.region_code, mbl.business_line_code, tdh.total_document, tdh.is_manual_upload from tr_document_h tdh left join tr_document_d tdd on tdd.id_Document_h = tdh.id_document_h left join ms_lov msl on tdd.lov_payment_sign_type = msl.id_lov left join ms_peruri_doc_type mpdt on tdd.id_peruri_doc_type = mpdt.id_peruri_doc_type left join ms_vendor msv on tdd.id_ms_vendor = msv.id_ms_vendor left join ms_office mso on tdh.id_ms_office = mso.id_ms_office left join ms_region msr on mso.id_ms_region = msr.id_ms_region left join ms_business_line mbl on tdh.id_ms_business_line = mbl.id_ms_business_line where tdh.ref_number = '" + refNumber + "'")
		metadata = resultSet.metaData
