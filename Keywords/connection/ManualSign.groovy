...
public class ManualSign {
	...
	getVendorofTenant(Connection conn, String tenantCode) {
		...
		resultSet = stm.executeQuery("select msv.vendor_name from ms_vendoroftenant mvot left join ms_tenant mst on mvot.id_ms_tenant = mst.id_ms_tenant left join ms_vendor msv on mvot.id_ms_vendor = msv.id_ms_vendor where mst.tenant_code = '"+tenantCode+"' and msv.is_active = '1' and mvot.default_vendor > '0' order by mvot.default_vendor asc")
