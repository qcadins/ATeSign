...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathGetInvData).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		respon_login = WS.sendRequest(findTestObject('Postman/Login', [('username') : findTestData(excelPathGetInvData).getValue(GlobalVariable.NumofColm, rowExcel('username'))
					, ('password') : findTestData(excelPathGetInvData).getValue(GlobalVariable.NumofColm, rowExcel('password'))]))
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			'HIT API'
			respon = WS.sendRequest(findTestObject('Postman/Get Data Invitation Registration', [('callerId') : '"' + findTestData(excelPathGetInvData).getValue(
						GlobalVariable.NumofColm, rowExcel('callerId')) + '"', ('loginid') : '"' + findTestData(excelPathGetInvData).getValue(
			...
			CustomKeywords.'customizekeyword.BeautifyJson.process'(responseBody, sheet, rowExcel('Respons') - 1,
				findTestData(excelPathGetInvData).getValue(GlobalVariable.NumofColm, rowExcel('Scenario')))
...
def getErrorMessageAPI(def respon) {
	...
	CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
		((findTestData(excelPathGetInvData).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + ';') + ('<' + message)) +
		'>')
