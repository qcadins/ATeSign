...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        respon = WS.sendRequest(findTestObject('Postman/Verify Otp Signing Embed', [('msg') : encryptMsg, ('tenantCode') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')), ('phoneNo') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('phoneNo')), ('otpCode') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('otpCode')), ('callerId') : findTestData(
