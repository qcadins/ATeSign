    ...
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (WS.verifyResponseStatusCode(responGetinvLink, 200, FailureHandling.OPTIONAL) == true) {
                'get Status Code'
                statusCode = WS.getElementPropertyValue(responGetinvLink, 'status.code')
                ...
                if (statusCode == 0) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, rowExcel(
                            'Links') - 1, GlobalVariable.NumofColm - 1, WS.getElementPropertyValue(responGetinvLink, 'invitationLink').toString())
