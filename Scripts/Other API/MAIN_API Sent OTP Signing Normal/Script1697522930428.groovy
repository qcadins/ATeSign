...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('use Correct Tenant Code')) == 'No') {
            ...
            GlobalVariable.Tenant = findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('$tenantCode'))
        }
		...
		documentId = findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('$documentId')).split(';', -1)
		...
		'HIT API'
		responLogin = WS.sendRequest(findTestObject('APIFullService - Privy/Postman/Login', [('email') : findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm,
			rowExcel('Email Login')).toString(), ('password') : findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('Password Login')).toString()]))
		...
		if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
			...
			if (findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('Use True Token')).equalsIgnoreCase(
				...
				'No')) {
				GlobalVariable.token = findTestData(excelPathAPISentOTPSigning).getValue(GlobalVariable.NumofColm, rowExcel('Wrong Token'))
			}
			
			'HIT API'
			respon = WS.sendRequest(findTestObject('Postman/Sent Otp Signing Normal', [('callerId') : ('"' + findTestData(excelPathAPISentOTPSigning).getValue(
				GlobalVariable.NumofColm, rowExcel('$callerId'))) + '"', ('phoneNo') : ('"' + findTestData(excelPathAPISentOTPSigning).getValue(
					GlobalVariable.NumofColm, rowExcel('$phoneNo'))) + '"', ('vendorCode') : ('"' + findTestData(excelPathAPISentOTPSigning).getValue(
						GlobalVariable.NumofColm, rowExcel('$vendorCode'))) + '"', ('tenantCode') : ('"' + findTestData(excelPathAPISentOTPSigning).getValue(
							GlobalVariable.NumofColm, rowExcel('$tenantcode'))) + '"', ('listDocumentId') : listDoc]))
			...
			if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
				...
				if (code == 0) {
					...
					if (GlobalVariable.Psre == 'PRIVY') {
						...
						CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet,
							rowExcel('OTP')-1, GlobalVariable.NumofColm - 1, 'OTP tidak masuk ke DB(PRIVY)')
					} else {
						...
						newResetNum = Integer.parseInt(CustomKeywords.'connection.APIFullService.getResetCodeRequestNum'(conneSign, findTestData(excelPathAPISentOTPSigning).getValue(
							GlobalVariable.NumofColm, rowExcel('Email Login'))))
