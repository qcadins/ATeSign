...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/businessLineList (saldo)', [
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (status_Code == 0) {
					...
					businessCode = WS.getElementPropertyValue(respon, 'businessLineList.businessLineCode', FailureHandling.OPTIONAL)

                    if (GlobalVariable.checkStoreDB == 'Yes' && businessCode.size() != 0) {
						...
						arrayMatch.add(WebUI.verifyEqual(result, businessCode.size(), FailureHandling.CONTINUE_ON_FAILURE))
