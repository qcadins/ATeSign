...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            respon = WS.sendRequest(findTestObject('Postman/ListPaymentType', [('callerId') : findTestData(excelPath).getValue(
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        ArrayList result = CustomKeywords.'connection.APIFullService.getListPaymentTypeAPIOnly'(conneSign, 
                            GlobalVariable.Tenant, GlobalVariable.Psre)

                        'deklarasi array index'
                        arrayIndex = 0

                        'verify code di API dengan DB'
                        arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], WS.getElementPropertyValue(respon, 'paymentSignTypeList[0].code', 
                                    FailureHandling.OPTIONAL).toString(), false, FailureHandling.CONTINUE_ON_FAILURE))

                        'verify description di API dengan DB'
                        arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], WS.getElementPropertyValue(respon, 'paymentSignTypeList[0].description', 
                                    FailureHandling.OPTIONAL).toString(), false, FailureHandling.CONTINUE_ON_FAILURE))
