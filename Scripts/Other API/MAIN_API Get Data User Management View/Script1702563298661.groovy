...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		'HIT API Login untuk token : andy@ad-ins.com'
		respon = WS.sendRequest(findTestObject('Postman/Get Data User Management View', [('tenantCode') : findTestData(excelPath).getValue(
			...
			if (status_Code == 0) {
				...
				if (GlobalVariable.checkStoreDB == 'Yes') {
					ArrayList result = CustomKeywords.'connection.UserManagement.getDataUserManagementViewAPI'(conneSign, 
						findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('role')), 
