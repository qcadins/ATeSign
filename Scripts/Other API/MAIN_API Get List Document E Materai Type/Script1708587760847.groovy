...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API Login untuk token : andy@ad-ins.com'
        respon = WS.sendRequest(findTestObject('Postman/Get List Document E Materai Type', [('callerId') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('callerId')), ('msg') : msg]))
...
def encrypt(Connection conneSign) {
    msg = (((((('{"tenantCode":"' + findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode'))) + 
    '","officeCode":"') + findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('officeCode'))) + '","email":"') + 
    ...
    msg = CustomKeywords.'customizekeyword.ParseText.parseEncrypt'(msg, aesKey)
