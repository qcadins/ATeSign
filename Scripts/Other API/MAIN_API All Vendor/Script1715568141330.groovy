...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        respon = WS.sendRequest(findTestObject('Postman/All Vendor', [('callerId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('callerId')), ('tenantCode') : findTestData(excelPath).getValue(
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ...
                    ArrayList<String> result = CustomKeywords.'connection.APIOnly.getAllVendor'(conneSign)

					'get Code'
					vendorCode = WS.getElementPropertyValue(respon, 'vendorList.code', FailureHandling.OPTIONAL)
					
					'get name'
					vendorName = WS.getElementPropertyValue(respon, 'vendorList.name', FailureHandling.OPTIONAL)
					...
					for (looping = 0; looping < result.size() / 2; looping++) {
						'verify vendor code'
						arrayMatch.add(WebUI.verifyMatch(vendorCode[looping], result[arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify vendor name'
						arrayMatch.add(WebUI.verifyMatch(vendorName[looping], result[arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
					}
