...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        respon = WS.sendRequest(findTestObject('Postman/Submit Result Rekon', [('callerId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('username')), ('tenantCode') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('tenantCode')), ('vendorCode') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('vendorCode')), ('balanceType') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('balanceType')), ('startDate') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('startDate')), ('endDate') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('endDate')), ('loginId') : findTestData(excelPath).getValue(
