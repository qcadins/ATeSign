    ...
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
       ...
       'get office code dari db'
		...
		if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Embed Version')).equalsIgnoreCase('V2')) {
			...
			url = 'embed/document/viewSignerEmbed'
        ...
        respon = WS.sendRequest(findTestObject('Postman/View Signer Embed', [('documentId') : endcodedDocumentId, ('msg') : endcodedMsg
                    , ('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('callerId')), ('url') : url]))
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (statusCode == 0) {
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ...
                    'get data store db'
                    ArrayList result = CustomKeywords.'connection.APIFullService.getViewSigner'(conneSign, findTestData(
                            excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document ID')))

                    'declare arrayindex'
                    arrayindex = 0

                    'verify total signer'
                    arrayMatch.add(WebUI.verifyMatch(resultTotalResult.toString(), (result.size() / 7).toString(), false, 
                    ...
                    for (index = 0; index < (result.size() / 7); index++) {
                        ...
                        arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], resultSignerRegisterStatus[index], false, 
                                FailureHandling.CONTINUE_ON_FAILURE))
                    }
