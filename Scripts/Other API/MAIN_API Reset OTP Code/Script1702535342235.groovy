...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, rowExcel('Reset Count before') - 1, GlobalVariable.NumofColm -
			1, CustomKeywords.'connection.APIFullService.getResetOtpCodeAPIOnly'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Email'))))
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (status_Code == 0) {
					...
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, rowExcel('Reset Count after') - 1, GlobalVariable.NumofColm -
						1, CustomKeywords.'connection.APIFullService.getResetOtpCodeAPIOnly'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Email'))))
					...
					if (WebUI.verifyNotMatch(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reset Count before')),
						findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reset Count after')), false, FailureHandling.OPTIONAL) ||
							findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reset Count after')) == '0') {
						...
						CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
							((findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + ';') + 'Hit sukses, namun reset count tidak ter-update'))
					}
                } else {
