...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		'HIT API Login untuk token : andy@ad-ins.com'
		respon = WS.sendRequest(findTestObject('Postman/Get List Data Pengguna', [('tenantCode') : findTestData(excelPath).getValue(
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			if (status_Code == 0) {
				...
				if (GlobalVariable.checkStoreDB == 'Yes') {
					ArrayList result = CustomKeywords.'connection.EditSignerData.getGetListDataPenggunaAPI'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('loginId')), findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')))
					
					resultLoginId = WS.getElementPropertyValue(respon, 'listDataPengguna.loginId', FailureHandling.OPTIONAL)
					
					resultFullName = WS.getElementPropertyValue(respon, 'listDataPengguna.fullName', FailureHandling.OPTIONAL)
					
					resultIsActive = WS.getElementPropertyValue(respon, 'listDataPengguna.isActive', FailureHandling.OPTIONAL)
					
					resultVendorCode = WS.getElementPropertyValue(respon, 'listDataPengguna.vendorCode', FailureHandling.OPTIONAL)
					
					resultIsRegistered = WS.getElementPropertyValue(respon, 'listDataPengguna.isRegistered', FailureHandling.OPTIONAL)
					
					resultIsActivated = WS.getElementPropertyValue(respon, 'listDataPengguna.isActivated', FailureHandling.OPTIONAL)
					
					resultVendorName = WS.getElementPropertyValue(respon, 'listDataPengguna.vendorName', FailureHandling.OPTIONAL)
					...
					for (index = 0; index < (result.size() / 7); index++) {
						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], resultLoginId[index], false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], resultFullName[index], false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++].toString().toLowerCase(), resultIsActive[index].toString().toLowerCase(), false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], resultVendorCode[index], false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], resultIsRegistered[index], false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], resultIsActivated[index], false, FailureHandling.CONTINUE_ON_FAILURE))
