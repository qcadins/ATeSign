...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        respon = WS.sendRequest(findTestObject('Postman/Get Sub District Inv', [('callerId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('callerId')), ('subDistrictName') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('subDistrictName')), ('districtId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('districtId')), ('msg') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('msg'))]))
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
			...
			ArrayList<String> districtName = WS.getElementPropertyValue(respon, 'listDistrict.districtName', FailureHandling.OPTIONAL)

			'get idMsDistrict'
			ArrayList<String> idMsDistrict = WS.getElementPropertyValue(respon, 'listDistrict.idMsDistrict', FailureHandling.OPTIONAL)
            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {	
					...
					ArrayList<String> result = CustomKeywords.'connection.APIOnly.getSubDistrict'(conneSign, findTestData(
							...
							GlobalVariable.NumofColm, rowExcel('districtId')))
					...
					if ((result.size() < 2) && (districtName.size() > 2)) {
						arrayMatch.add(false)
					} else {
						for (i = 0; i < districtName.size(); i++) {
							'verify get sub district'
							arrayMatch.add(WebUI.verifyMatch(districtName[i], result[arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))

							'verify get sub district'
							arrayMatch.add(WebUI.verifyMatch(idMsDistrict[i], result[arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
						}
					}
