		...
		if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
			...
			if (status_Code == 0) {
				...
				if ((GlobalVariable.checkStoreDB == 'Yes') && (GlobalVariable.FlagFailed == 0)) {
					...
					'verify nik'
					arrayMatch.add(WebUI.verifyMatch(CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(WS.getElementPropertyValue(respon, 'idNo', FailureHandling.OPTIONAL)),
						result[arrayindex++], false, FailureHandling.CONTINUE_ON_FAILURE))
					
					'verify phoneNo'
					arrayMatch.add(WebUI.verifyMatch(CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(WS.getElementPropertyValue(respon, 'phoneNo', FailureHandling.OPTIONAL)),
						result[arrayindex++], false, FailureHandling.CONTINUE_ON_FAILURE))
