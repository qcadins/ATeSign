...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/listTenant(admesign)', [
						...
						('tenantName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Tenant Name*')), 
						('isActive') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status*'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    ...
                    if (((GlobalVariable.checkStoreDB == 'Yes') && (totalResult != 0)) && (totalResult != '')) {
                        ...
                        'get data store db'
                        int result = CustomKeywords.'connection.APIFullService.listTenantAPIOnly'(conneSign, findTestData(
                                excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Tenant Name*')), findTestData(excelPath).getValue(
                                GlobalVariable.NumofColm, rowExcel('Status*')))
