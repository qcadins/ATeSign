...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			'HIT API'
			respon = WS.sendRequest(findTestObject('Postman/Get Tenant Settings', [('tenantCode'): '"' + GlobalVariable.Tenant + '"']))
			...
			if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
				...
				if (status_Code == 0) {
					...
					if ((GlobalVariable.checkStoreDB == 'Yes')) {
						...
						arrayMatch.add(WebUI.verifyMatch(WS.getElementPropertyValue(respon, 'thresholdBalance', FailureHandling.OPTIONAL).toString().replace('.0','').replace(' ','').replace('{','').replace('}','').replace('=',':'),
							result[arrayindex++].toString().replace('{','').replace('}','').replace('"',''), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify emailReminderDest'
						arrayMatch.add(WebUI.verifyMatch(WS.getElementPropertyValue(respon, 'emailReminderDest', FailureHandling.OPTIONAL).toString().replace(' ','').replace('[','').replace(']',''),
							result[arrayindex++], false, FailureHandling.CONTINUE_ON_FAILURE))
