...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API Login untuk token : andy@ad-ins.com'
        respon = WS.sendRequest(findTestObject('Postman/Get Role List', [('tenantCode') : findTestData(excelPath).getValue(
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (status_Code == 0) {
				...
				if (GlobalVariable.checkStoreDB == 'Yes') {
					resultRole = CustomKeywords.'connection.UserManagement.getRoleUserManagementAPI'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')))
					
					resultRoleName = WS.getElementPropertyValue(respon, 'listRole.roleName', FailureHandling.OPTIONAL)
					
					resultRoleCode = WS.getElementPropertyValue(respon, 'listRole.roleCode', FailureHandling.OPTIONAL)
					
					arrayIndex = 0
					
					ArrayList arrayMatch = []
					
					for (index = 0; index < (resultRole.size() / 2); index++) {
						'verify signer type'
						arrayMatch.add(WebUI.verifyMatch(resultRole[arrayIndex++], resultRoleName[index], false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify signer name'
						arrayMatch.add(WebUI.verifyMatch(resultRole[arrayIndex++], resultRoleCode[index], false, FailureHandling.CONTINUE_ON_FAILURE))
					}
