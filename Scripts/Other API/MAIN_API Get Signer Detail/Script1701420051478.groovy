			...
			if (WS.verifyResponseStatusCode(responSignerDetail, 200, FailureHandling.OPTIONAL) == true) {
				...
				if (status_Code == 0) {
					...
					if ((GlobalVariable.checkStoreDB == 'Yes') && (GlobalVariable.FlagFailed == 0)) {
						...
						'verify phoneNo'
						arrayMatch.add(WebUI.verifyMatch(CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(WS.getElementPropertyValue(responSignerDetail, 'phoneNo', FailureHandling.OPTIONAL)),
						...
						arrayMatch.add(WebUI.verifyMatch(WS.getElementPropertyValue(responSignerDetail, 'maxLivenessFaceCompareAttempt', FailureHandling.OPTIONAL),
							result[arrayindex++], false, FailureHandling.CONTINUE_ON_FAILURE))
