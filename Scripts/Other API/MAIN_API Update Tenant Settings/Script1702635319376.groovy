...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		respon = WS.sendRequest(findTestObject('Postman/Update Tenant Settings', [('tenantCode') : findTestData(excelPath).getValue(
						...
						, ('refNumberLabel') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('refNumberLabel'))
						, ('thresholdBalance') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('thresholdBalance'))
						, ('emailReminderDest') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('emailReminderDest'))
						, ('activationCallbackUrl') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('activationCallbackUrl'))
						, ('automaticSign') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('automaticSign'))
						, ('clientCallbackUrl') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('clientCallbackUrl'))
						, ('clientSigningRedirectUrl') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('clientSigningRedirectUrl'))
						, ('useWaMessage') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('useWaMessage'))]))
