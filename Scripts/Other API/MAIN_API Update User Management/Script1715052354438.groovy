...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        respon = WS.sendRequest(findTestObject('Postman/Update User Management', [('callerId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('callerId')), ('loginId') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('loginId')), ('isActive') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('isActive')), ('officeCode') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('officeCode')), ('tenantCode') : findTestData(excelPath).getValue(
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    'db result'
                    result = CustomKeywords.'connection.UserManagement.getUserManagementonEditAPI'(conneSign, findTestData(
                            excelPath).getValue(GlobalVariable.NumofColm, rowExcel('loginId')).toUpperCase(), 
                    ...
                    'verify login id'
                    arrayMatch.add(WebUI.verifyMatch(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('loginId')).toUpperCase(), result[arrayIndex++].toUpperCase(), false, FailureHandling.CONTINUE_ON_FAILURE))

					'verify role code'
					arrayMatch.add(WebUI.verifyMatch(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('roleCode')).toUpperCase(), result[arrayIndex++].toUpperCase(), false, FailureHandling.CONTINUE_ON_FAILURE))
					...
					'verify is active'
					arrayMatch.add(WebUI.verifyMatch(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel(
									'isActive')), result[arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
