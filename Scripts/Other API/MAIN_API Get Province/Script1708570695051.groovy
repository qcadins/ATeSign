...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'get sub province name'
            ArrayList<String> namaProvinsi = WS.getElementPropertyValue(respon, 'provinceList.namaProvinsi', FailureHandling.OPTIONAL)

            'get idMsProvinsi'
            ArrayList<String> idMsProvinsi = WS.getElementPropertyValue(respon, 'provinceList.idMsProvinsi', FailureHandling.OPTIONAL)
            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ...
                    if ((result.size() < 2) && (namaProvinsi.size() > 2)) {
