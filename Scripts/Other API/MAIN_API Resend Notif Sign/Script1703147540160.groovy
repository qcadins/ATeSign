...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/resendNotifSign', [
						('docId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document ID')), 
...
def checkBalanceMutation(Connection conneSign) {
		...
		if (emailServiceOnTenant == 1) {
			...
			if (useWAMessage == '1') {
				...
				if (balmut.size() == 0) {
					...
					'Jika equalnya salah maka langsung berikan reason bahwa reasonnya failed'
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
						((findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + ';') + 'Tidak ada transaksi yang terbentuk ketika melakukan pengiriman OTP Via SMS'))
				}
				...
				if (balmut[8] != (-1)) {
					...
					'Jika equalnya salah maka langsung berikan reason bahwa reasonnya failed'
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
						((findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + ';') + 'Saldo SMS tidak terpotong'))
				}
			}
		}
	}
	...
	if (GlobalVariable.checkStoreDB == 'Yes' &&
		...
		ArrayList resultDB = CustomKeywords.'connection.DataVerif.getBusinessLineOfficeCode'(conneSign,
			findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('username')), 'Document')
