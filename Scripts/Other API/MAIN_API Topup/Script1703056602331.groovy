...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/TopUp', [
						('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('username')), 
						('balanceTypeCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('BalanceType Code')), 
						('notes') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Notes')), 
						('qty') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('qty')), 
						('refNo') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Ref Number')), 
						('trxDate') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Trx Date'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        ArrayList result = CustomKeywords.'connection.APIFullService.getTopUpAPIONLY'(conneSign, findTestData(
                                excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Ref Number')))
