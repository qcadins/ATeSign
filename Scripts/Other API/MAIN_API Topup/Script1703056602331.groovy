...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/TopUp', [('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('username')), ('balanceTypeCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('BalanceType Code')), ('notes') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Notes')), ('qty') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('qty')), ('refNo') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Ref Number')), ('trxDate') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Trx Date'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (status_Code == 0) {
                    ...
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        ArrayList<String> result = CustomKeywords.'connection.APIFullService.getTopUpAPIONLY'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Ref Number')))
						...
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Tenant Code')), false, FailureHandling.CONTINUE_ON_FAILURE))
						...
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('BalanceType Code')), false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify Note'
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Notes')), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify qty'
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('qty')), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify ref number'
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Ref Number')), false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify trx date'
						arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Trx Date')), false, FailureHandling.CONTINUE_ON_FAILURE))
