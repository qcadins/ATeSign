...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API Login untuk token : andy@ad-ins.com'
        respon = WS.sendRequest(findTestObject('Postman/Error History Act Status', [
						('tenantCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')), 
						('idErrorHistory') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('idErrorHistory')), 
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ArrayList result = CustomKeywords.'connection.ErrorReport.getErrorHistoryActStatus'(conneSign, findTestData(
                            excelPath).getValue(GlobalVariable.NumofColm, rowExcel('idErrorHistory')))
                    ...
                    ArrayList resultActivationStatus = WS.getElementPropertyValue(respon, 'listActStatus.activationStatus', 
                        FailureHandling.OPTIONAL)

                    ArrayList resultSignerType = WS.getElementPropertyValue(respon, 'listActStatus.signerType', FailureHandling.OPTIONAL)

                    ArrayList arrayMatch = [], resultStatusActivation = []
                    ...
                    for (index = 0; index < (result.size() / 4); index++) {
                        ...
                        resultStatusActivation = CustomKeywords.'connection.ErrorReport.getStatusActivationAPI'(conneSign, 
                            resultName[index], findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('idErrorHistory')))

                        'verify signer sign status'
                        arrayMatch.add(WebUI.verifyMatch(resultActivationStatus[index], resultStatusActivation[0], false, 
