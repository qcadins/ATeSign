...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/getStatusStampingOtomatisTenant (Manual Sign)', [
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        String result = CustomKeywords.'connection.APIFullService.getAutoStampTenantAPIOnly'(conneSign, 
                            GlobalVariable.Tenant)

                        'verify status di API dengan DB'
                        arrayMatch.add(WebUI.verifyMatch(result, WS.getElementPropertyValue(respon, 'statusStampingOtomatis', 
