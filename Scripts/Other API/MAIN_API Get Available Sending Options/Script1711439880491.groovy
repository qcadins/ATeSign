...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'HIT API'
        responLogin = WS.sendRequest(findTestObject('Postman/Login', [('username') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('Email Login')), ('password') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('Password Login'))]))

        'Jika status HIT API 200 OK'
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Use True Token')) != 'No') {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/getAvailableSendingOptions', [('tenantCode') : findTestData(
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        ArrayList resultListAvailable = CustomKeywords.'connection.APIOnly.listAvailableOptionSendingPointGetAvailSendingPointInv'(
                            conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')))
                        ...
                        arrayMatch.add(WebUI.verifyMatch(resultListAvailable.toString(), listAvailableOptionSendingPoint.toString(), 
                                false, FailureHandling.CONTINUE_ON_FAILURE))

						'jika default otpnya bukan sms dan wa, maka akan automate ke sms'
...
def getNotifType(Connection conneSign, String emailSigner) {
    ...
    String notifTypeDB = CustomKeywords.'connection.APIFullService.getWASMSFromNotificationType'(conneSign, emailSigner, 
