...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		listInvitation[0] =
		'{"email": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('email')) + '",' +
		'"tlp": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('tlp')).toString() + '",' +
		'"jenisKelamin": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('jenisKelamin')) + '",' +
		'"tmpLahir": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('tmpLahir')) + '",' +
		'"tglLahir": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('tglLahir')) + '",' +
		'"idKtp": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('idKtp')) + '",' +
		'"provinsi": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('provinsi')) + '",' +
		'"kota": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('kota')) + '",' +
		'"kecamatan": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('kecamatan')) + '",' +
		'"kelurahan": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('kelurahan')) + '",' +
		'"kodePos": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('kodePos')) + '",' +
		'"alamat": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('alamat')) + '",' +
		'"selfPhoto": "",' +
		'"idPhoto": "",' +
		'"region": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Wilayah')) + '",' +
		'"office": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Office')) + '",' +
		'"businessLine": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Lini Bisnis')) + '",' +
		'"taskNo": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Task No')) + '",' +
		'"nama": "' + findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('nama')) + '"}';
		
		'HIT API Login untuk ambil bearer token'
		respon_login = WS.sendRequest(findTestObject('Postman/Login', [('username') : findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('username'))
					, ('password') : findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('password'))]))
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			'HIT API'
			respon = WS.sendRequest(findTestObject('Postman/Generate Invitation Link Secured', [('callerId') : '"' + findTestData(excelPathGenerateLink).getValue(
			...
			CustomKeywords.'customizekeyword.BeautifyJson.process'(responseBody, sheet, rowExcel('Respons') - 1,
				findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Scenario')))
			...
			if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
				...
				if (status_Code == 0) {
					...
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, rowExcel('Links') - 1, GlobalVariable.NumofColm -
						1, links.toString().replace('[','').replace(']',''))
...
def getErrorMessageAPI(def respon) {
	...
	CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
		((findTestData(excelPathGenerateLink).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + ';') + ('<' + message)) +
		'>')
