            ...
            if (statusCode == 0) {
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ...
                    String docId = CustomKeywords.'connection.DataVerif.getDocId'(conneSign, findTestData(excelPath).getValue(
                            GlobalVariable.NumofColm, rowExcel('refNumber')), findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('tenantCode')))
                    ...
                    if (docId != null) {
                        ...
                        CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                            GlobalVariable.StatusFailed, (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel(
                                    'Reason Failed')).replace('-', '') + ';') + GlobalVariable.ReasonFailedStoredDB)
