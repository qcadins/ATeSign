...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/listDocumentTemplate', [('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('username')),('page') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Page*')), ('docTempCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Document Template Code*')),('docTempName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Document Template Name*')),('isActive') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('isActive*'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (status_Code == 0) {
                    ...
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        String result = CustomKeywords.'connection.APIFullService.listDocTemplateAPIOnly'(conneSign, GlobalVariable.Tenant,
							findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document Template Code*')),
							findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document Template Name*')),
							findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('isActive*')))

						'verify status di API dengan DB'
						arrayMatch.add(WebUI.verifyMatch(result, WS.getElementPropertyValue(respon, 'totalResult', FailureHandling.OPTIONAL).toString(), false, FailureHandling.CONTINUE_ON_FAILURE))
