...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/listDocumentTemplate', [
							...
							('docTempCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document Template Code*')), 
							('docTempName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Document Template Name*')), 
							('isActive') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('isActive*'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        arrayMatch.add(WebUI.verifyMatch(result, WS.getElementPropertyValue(respon, 'totalResult', FailureHandling.OPTIONAL).toString(), 
