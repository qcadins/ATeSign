...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(responLogin, 200, FailureHandling.OPTIONAL) == true) {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/tenant(admesign)', [
						('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('username')), 
						('isActive') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status Active*'))]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    tenantListCode = WS.getElementPropertyValue(respon, 'tenantList.tenantCode', FailureHandling.OPTIONAL)

                    if ((GlobalVariable.checkStoreDB == 'Yes') && (tenantListCode.size() != 0)) {
                        'declare arraylist arraymatch'
                        ArrayList arrayMatch = []

                        'get data store db'
                        int result = CustomKeywords.'connection.APIFullService.listTenantAPIOnly'(conneSign, '', findTestData(
                                excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status Active*')))

                        'verify livenessFacecompareServicesStatus'
                        arrayMatch.add(WebUI.verifyEqual(result, tenantListCode.size(), FailureHandling.CONTINUE_ON_FAILURE))
