...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		respon = WS.sendRequest(findTestObject('Postman/Update Data Signer', [('tenantCode') : findTestData(excelPath).getValue(
						...
						('vendorCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('vendorCode')),
						('fullName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('fullName')),
						('dateOfBirth') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('dateOfBirth')),
						('noKtp') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noKtp')),
						('noPhone') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noPhone')),
						('newEmail') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('newEmail'))						
						]))
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			if (status_Code == 0) {
				if (GlobalVariable.checkStoreDB == 'Yes') {
					...
					ArrayList result = CustomKeywords.'connection.EditSignerData.getUpdateDataSignerAPI'(conneSign,
						findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('loginId')),
						findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tenantCode')),
						findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('vendorCode')))
					...
					for (index = 0; index < result.size() / 5; index++) {
						if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('fullName')) != '') {
							'verify full name'
							arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('fullName')), false, FailureHandling.CONTINUE_ON_FAILURE))
						} else {
							arrayIndex++
						}
						...
						if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('newEmail')) != '') {
							'verify full name'
							arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('newEmail')), false, FailureHandling.CONTINUE_ON_FAILURE))
						} else {
							'verify full name'
							arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('loginId')), false, FailureHandling.CONTINUE_ON_FAILURE))
						}
						
						if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noPhone')) != '') {
							'verify full name'
							arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noPhone'))), false, FailureHandling.CONTINUE_ON_FAILURE))
						} else {
							arrayIndex++
						}
						...
						if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noKtp')) != '') {
							'verify full name'
							arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('noKtp'))), false, FailureHandling.CONTINUE_ON_FAILURE))
						} else {
							arrayIndex++
						}
						...
						'verify full name'
						arrayMatch.add(WebUI.verifyMatch(result[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('dateOfBirth')), false, FailureHandling.CONTINUE_ON_FAILURE))
						} else {
							arrayIndex++
						}
					}
