...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		respon = WS.sendRequest(findTestObject('Postman/ListInvitationReport', [('tenantCode') : findTestData(excelPath).getValue(
						...
						, ('nama') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('nama'))
						, ('penerimaUndangan') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('penerimaUndangan'))
						, ('tanggalPengirimanDari') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tanggalPengirimanDari'))
						, ('tanggalPengirimanSampai') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tanggalPengirimanSampai'))
						, ('statusUndangan') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('statusUndangan'))
						, ('pengirimanMelalui') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('pengirimanMelalui'))
						, ('statusRegistrasi') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('statusRegistrasi'))]))
