...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			def elapsedTime = (responResendInv.getElapsedTime()) / 1000 + ' second'
			
			'ambil body dari hasil respons'
			responseBody = responResendInv.getResponseBodyContent()
			...
			if (WS.verifyResponseStatusCode(responResendInv, 200, FailureHandling.OPTIONAL) == true) {
				'get Status Code'
				status_Code = WS.getElementPropertyValue(responResendInv, 'status.code')
				...
				if (status_Code == 0) {
					...
					getErrorMessageAPI(responResendInv)
				}
			} else {
				getErrorMessageAPI(responResendInv)
			}
