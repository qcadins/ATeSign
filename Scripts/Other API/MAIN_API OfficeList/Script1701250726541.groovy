            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (statusCode == 0) {
                    ...
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        'get data store db'
                        ArrayList result = CustomKeywords.'connection.APIFullService.getOfficeList'(conneSign, findTestData(
                                excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Region Code')))
                        ...
                        for (index = 0; index < (result.size() / 2); index++) {
                            'verify vendor code'
                            arrayMatch.add(WebUI.verifyMatch(result[arrayindex++], resultOfficeCode[index], false, FailureHandling.CONTINUE_ON_FAILURE))
