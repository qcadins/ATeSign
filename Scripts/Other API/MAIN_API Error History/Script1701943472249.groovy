...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
	println findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status'))
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        respon = WS.sendRequest(findTestObject('Postman/Error History', [('tenantCode') : findTestData(excelPath).getValue(
                        ...
                        rowExcel('page')), 
					('modul') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('modul')),
					('namaKonsumen') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('namaKonsumen')),
					('businessLine') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('businessLine')),
					('cabang') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('cabang')),
					('region') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('region')),
					('tanggalDari') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tanggalDari')),
					('tanggalSampai') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tanggalSampai')),
					('tipe') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('tipe')),
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (status_Code == 0) {
				...
				if (GlobalVariable.checkStoreDB == 'Yes') {
					...
					for (index = 0; index < listErrorHistory.size(); index++) {
					}
				}
