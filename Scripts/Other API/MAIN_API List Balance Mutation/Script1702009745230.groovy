    ...
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
		    ...
		    GlobalVariable.token = WS.getElementPropertyValue(respon_login, 'access_token')
			 ...
			 'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/List Balance Mutation (Saldo)', [('pageNum') : (findTestData(excelPath).getValue(
                ...
                GlobalVariable.NumofColm, rowExcel('Transaction Date Start*'))), ('tranxDateEnd') : (findTestData(excelPath).getValue(
                GlobalVariable.NumofColm, rowExcel('Transaction Date End*'))), ('docType') : (findTestData(excelPath).getValue(
                GlobalVariable.NumofColm, rowExcel('Document Type*'))), ('callerId') : (findTestData(excelPath).getValue(
