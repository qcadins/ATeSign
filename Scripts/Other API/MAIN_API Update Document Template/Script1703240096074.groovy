...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			signerTypeCode = findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('signerTypeCode')).split(';', -1)
			
			'Inisialisasi signTypeCode'
			signTypeCode = findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('signTypeCode')).split(';', -1)
			...
			for (index = 1; index <= signerTypeCode.size(); index++) {
				if (index < signerTypeCode.size()) {
					signerBody = signerBody + '{ "signerTypeCode": "' + signerTypeCode[index-1] + '", "signTypeCode": "' + signTypeCode[index-1] + '", "signPage": "' + signPage[index-1] + '", "signLocation": ' + signLocation[index-1] + ', "transform": "' + transform[index-1] + '", "position": "' + position[index-1] + '", "positionVida": "' + positionVida[index-1] + '", "positionPrivy": "' + positionPrivy[index-1] + '", "seqNo": "' + seqNo[index-1] +'"},'
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/Update Document Template', [('callerId') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('username')), ('documentTemplateCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('documentTemplateCode')), ('documentTemplateName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('documentTemplateName')), ('documentTemplateDescription') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('documentTemplateDescription')), ('isActive') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('isActive')), ('isSequence') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('isSequence')), ('numberOfPage') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('numberOfPage')), ('paymentSignTypeCode') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('paymentSignTypeCode')), ('isSignLocOnly') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                            rowExcel('isSignLocOnly')), ('documentExample') : documentFile, ('signer') : signerBody]))
            ...
            if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
                ...
                if (status_Code == 0) {
					...
					if (GlobalVariable.checkStoreDB == 'Yes') {
						ArrayList resultDataSigner = CustomKeywords.'connection.PengaturanDokumen.getDocumentTemplateAPI'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('documentTemplateCode')))
						
						ArrayList resultDataDocTemplate = CustomKeywords.'connection.PengaturanDokumen.getDocumentTemplateAPI2'(conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('documentTemplateCode')))
						...
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('documentTemplateCode')), false, FailureHandling.CONTINUE_ON_FAILURE))
					
						'verify documentTemplateName'
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('documentTemplateName')), false, FailureHandling.CONTINUE_ON_FAILURE))

						'verify documentTemplateDescription'
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('documentTemplateDescription')), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify isActive'
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('isActive')), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify paymentSignTypeCode'
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('paymentSignTypeCode')), false, FailureHandling.CONTINUE_ON_FAILURE))
						
						'verify isSequence'
						arrayMatch.add(WebUI.verifyMatch(resultDataDocTemplate[arrayIndex++], findTestData(excelPath).getValue(GlobalVariable.NumofColm,
							rowExcel('isSequence')), false, FailureHandling.CONTINUE_ON_FAILURE))
						...
						for (index = 0; index < resultDataSigner.size() / 9; index++) {
							...
							'verify signLocation'
							arrayMatch.add(WebUI.verifyMatch(resultDataSigner[arrayIndex++], signLocation[index], false, FailureHandling.CONTINUE_ON_FAILURE))
						
							'verify signerTypeCode'
							arrayMatch.add(WebUI.verifyMatch(resultDataSigner[arrayIndex++], signerTypeCode[index], false, FailureHandling.CONTINUE_ON_FAILURE))
							
							'verify signTypeCode'
							arrayMatch.add(WebUI.verifyMatch(resultDataSigner[arrayIndex++], signTypeCode[index], false, FailureHandling.CONTINUE_ON_FAILURE))
							...
							'verify positionVida'
							arrayMatch.add(WebUI.verifyMatch(resultDataSigner[arrayIndex++], positionVida[index], false, FailureHandling.CONTINUE_ON_FAILURE))
							
							'verify positionPrivy'
							arrayMatch.add(WebUI.verifyMatch(resultDataSigner[arrayIndex++], positionPrivy[index], false, FailureHandling.CONTINUE_ON_FAILURE))
