...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Embed Version')).equalsIgnoreCase('V2')) {
            ...
            url = 'embed/document/signDocumentEmbed'
            ...
            url = 'document/signDocumentEmbed'
        }
        ...
        if (aesKey.toString() != 'null') {
            ...
            if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Use Correct DocumentID')) == 'No') {
                ...
                endcodedDocumentId = findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('documentId'))
        ...
        if (WS.verifyResponseStatusCode(responsigndocEmbed, 200, FailureHandling.OPTIONAL) == true) {
            'get Status Code'
            statusCode = WS.getElementPropertyValue(responsigndocEmbed, 'status.code')

			'ambil lama waktu yang diperlukan hingga request menerima balikan'
			elapsedTime = ((responsigndocEmbed.elapsedTime / 1000) + ' second')
	
			'ambil body dari hasil respons'
			responseBody = responsigndocEmbed.responseBodyContent
            ...
            if (statusCode == 0) {
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    for (timeout = 1; timeout <= 5; timeout++) {
                        ...
                        result = CustomKeywords.'connection.SendSign.getSignDocEmbedStoreDB'(conneSign, findTestData(excelPath).getValue(
