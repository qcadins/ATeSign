...
//        catatanStamping = findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 27).split(semicolon, splitIndex)
        ...
        if (WebUI.verifyElementPresent(findTestObject('ManualStamp/lbl_stampingMeterai'), GlobalVariable.TimeOut)) {
            ...
            if (findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 14) == 'Yes') {
            ...
            for (int i = 0; i < emailPenandaTangan.size(); i++) {
                ...
                queryCheckInformationUser = CustomKeywords.'connection.ManualSign.getInformationUser'(conneSign, emailPenandaTangan[indexEmail++], findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 8))
            ...
            if (WebUI.verifyElementPresent(findTestObject('ManualSign/lbl_documentNo'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
                ...
                checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('ManualSign/lbl_documentNo'), 
                            'value'), findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 9), false, FailureHandling.CONTINUE_ON_FAILURE), 
                ...
                checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('ManualSign/input_documentName'), 
                            'value'), findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 10), false, FailureHandling.CONTINUE_ON_FAILURE), 
            ...
            if (GlobalVariable.FlagFailed == 0) {
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    result = CustomKeywords.'connection.ManualSign.getManualSign'(conneSign, findTestData(excelPathManualStamp).getValue(
                    ...
                    if (findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 14) == 'Yes') {
                        tipeDokumenPeruri = findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 16)

                        if (findTestData(excelPathManualStamp).getValue(GlobalVariable.NumofColm, 17) == 'Ya') {
...
def inputCancel(Connection conneSign) {
    ...
    checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('ManualStamp/input_documentNo'), 'value', FailureHandling.OPTIONAL), 
    ...
    checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('ManualStamp/input_docType'), 'value', 
...
def zoomSetting(int percentage) {
	...
	//set the zoom to 70%
	((JavascriptExecutor)driver).executeScript("chrome.settingsPrivate.setDefaultZoom("+percentageZoom.toString()+");");
