...
import com.ibm.media.bean.multiplayer.LinksArrayEditor
...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(API_Excel_Path).getColumnNumbers(); (GlobalVariable.NumofColm)++) 
{
    ...
    if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) 
	{
		...
		if (status_Code == 0) 
		{
			...
			if (WebUI.verifyElementPresent(findTestObject('Object Repository/InquiryInvitation/button_ViewLink'), 2, FailureHandling.CONTINUE_ON_FAILURE))
			{
				...
				invitation_link_inquiry = WebUI.getAttribute(findTestObject('Object Repository/InquiryInvitation/input_Link'),'value',FailureHandling.CONTINUE_ON_FAILURE)
			...
			if (WebUI.verifyMatch(links.toString().replace('[', "").replace(']', ""), invitation_link_inquiry, false , FailureHandling.CONTINUE_ON_FAILURE)) 
			{
