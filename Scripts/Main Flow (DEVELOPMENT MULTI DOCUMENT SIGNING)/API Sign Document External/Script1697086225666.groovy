...
String refNumber = CustomKeywords.'connection.APIFullService.getRefNumber'(conneSign, documentId[GlobalVariable.storeVar.keySet()[[0]]])
	
documentIdInput = '["' + documentId[GlobalVariable.storeVar.keySet()[[0]]]  + '"]'
   ...
   HashMap<String, String> saldoBefore = WebUI.callTestCase(findTestCase('Main Flow/getSaldo'), [('excel') : excelPathAPISignDocument
	   ...
	   if (vendor.equalsIgnoreCase('Privy') || vendor.equalsIgnoreCase('Digisign')) {
        ...
        respon_OTP = WS.sendRequest(findTestObject('APIFullService/Postman/Sent Otp Signing', [('callerId') : findTestData(
                    ...
                    , ('email') : findTestData(excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, rowExcel('email (Sign External)')).split(
                        ...
                        conneSign, documentId[GlobalVariable.storeVar.keySet()[[0]]]) + '"']))
    ...
    if (otp.replace('"', '').length() >= 0) {
        ...
        'looping berdasarkan ukuran dari dokumen id'
            ...
            (totalSignedBefore[GlobalVariable.storeVar.keySet()[[0]]]) = CustomKeywords.'connection.APIFullService.getTotalSigned'(conneSign, (documentId[GlobalVariable.storeVar.keySet()[[0]]]).toString())
        ...
        respon = WS.sendRequest(findTestObject('APIFullService/Postman/Sign Document', [('callerId') : findTestData(excelPathAPISignDocument).getValue(
                    ...
                    , ('documentId') : documentIdInput, ('email') : findTestData(excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, 
                        rowExcel('email (Sign External)')).split(';', -1)[GlobalVariable.indexUsed], ('password') : '"' + findTestData(excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, rowExcel('Password Signer')) + '"'
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (code == 0) {
                    ...
                    for (int v = 1; v <= 20; v++) {
                        ...
                        (totalSignedAfter[GlobalVariable.storeVar.keySet()[[0]]]) = CustomKeywords.'connection.APIFullService.getTotalSigned'(conneSign, documentId[GlobalVariable.storeVar.keySet()[[0]]])

                        'Verify total signed sebelum dan sesudah. Jika sesuai maka break'
                        if ((totalSignedAfter[GlobalVariable.storeVar.keySet()[[0]]]) == ((totalSignedBefore[GlobalVariable.storeVar.keySet()[[0]]]) + Integer.parseInt(signCount))) {
                            WebUI.verifyEqual(totalSignedAfter[GlobalVariable.storeVar.keySet()[[0]]], (totalSignedBefore[GlobalVariable.storeVar.keySet()[[0]]]) + Integer.parseInt(signCount), 
							...
							'looping untuk mendapatkan total saldo yang digunakan per nomor kontrak'
								nomorKontrak = CustomKeywords.'connection.APIFullService.getRefNumber'(conneSign, documentId[GlobalVariable.storeVar.keySet()[0]])
								...
								if (paymentType == 'Per Sign') {
									...
									saldoUsed = (CustomKeywords.'connection.APIFullService.getSaldoUsedBasedonPaymentTypeMULTIDOC'(conneSign,
										documentId[GlobalVariable.storeVar.keySet()[0]], findTestData(excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, 
