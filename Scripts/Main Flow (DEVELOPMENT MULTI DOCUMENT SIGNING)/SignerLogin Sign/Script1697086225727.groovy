...
ArrayList documentIdFromNoKontrak = []

'bikin variable hashmap untuk ambil count per signer'
LinkedHashMap resultHashMap = new LinkedHashMap()
...
for (o = 0; o < forLoopingWithBreakAndContinue; o++) {
    String refNumber = CustomKeywords.'connection.APIFullService.getRefNumber'(conneSign, documentId[(GlobalVariable.storeVar.keySet()[
        [0]])])

    'looping berdasarkan jumlah dokumen'
    resultHashMap = loopingMultiDoc(documentId, conneSign, refNumber, resultHashMap)

    if (decisionForMultiDoc(resultHashMap) == false) {
        break
    }
    ...
    ArrayList sendToSign = CustomKeywords.'connection.APIFullService.getDataSendtoSign'(conneSign, documentId[(GlobalVariable.storeVar.keySet()[
        [0]])])
    ...
    if ((findTestData(excelPathFESignDocument).getValue(GlobalVariable.NumofColm, rowExcel('Bulk Signing ? (Yes/No)')).split(
        ...
        'Total document sign sudah dihitung berdasarkan multi document berdasarkan email yang login'
        totalDocSign = resultHashMap.get(GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[0]))
    }
    ...
    if (!(vendor.equalsIgnoreCase('Digisign'))) {
        ...
        if (WebUI.verifyElementPresent(findTestObject('KotakMasuk/Sign/lbl_VerifikasiOTPBerhasildanMasukan'), GlobalVariable.TimeOut, 
            ...
            noKontrakPerDoc = noKontrak.split(';', -1).toUnique()

            'looping untuk mendapatkan total saldo yang digunakan per nomor kontrak'
            for (i = 0; i < noKontrakPerDoc.size(); i++) {
                documentIdFromNoKontrak = CustomKeywords.'connection.SendSign.getDocumentIdBasedOnSigner'(conneSign, noKontrakPerDoc[
                    i], tenantCode, GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[0]))

                saldoForCheckingDB = 0

                for (loopingPerDoc = 0; loopingPerDoc < documentIdFromNoKontrak.size(); loopingPerDoc++) {
                    ...
                    for (y = 1; y <= 10; y++) {
						...
						jumlahSignerTandaTangan = CustomKeywords.'connection.APIFullService.getTotalSigned'(conneSign, documentIdFromNoKontrak[
							loopingPerDoc])
						
						jumlahHarusTandaTangan = CustomKeywords.'connection.APIFullService.getTotalSign'(conneSign, documentIdFromNoKontrak[
							loopingPerDoc])
						
						'Mengambil value dari db mengenai tipe pembayran'
						paymentType = CustomKeywords.'connection.APIFullService.getPaymentTypeMULTIDOC'(conneSign, documentIdFromNoKontrak[
							loopingPerDoc])
	
						saldoForCheckingDB = CustomKeywords.'connection.APIFullService.getSaldoUsedBasedonPaymentTypeMULTIDOC'(
							conneSign, documentIdFromNoKontrak[loopingPerDoc], GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[
								0]))
						...
						println jumlahSignerTandaTangan
						println jumlahHarusTandaTangan
                        ...
                        if (signingProcessStoreDB(conneSign, GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[
                                0]), saldoForCheckingDB, documentIdFromNoKontrak[loopingPerDoc]) == false) {
        ...
        WebUI.callTestCase(findTestCase('Main Flow/Signing Digisign'), [('excelPathFESignDocument') : excelPathFESignDocument
                , ('sheet') : sheet, ('nomorKontrak') : noKontrak, ('documentId') : documentId[(GlobalVariable.storeVar.keySet()[
                0])], ('emailSigner') : GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[0])], FailureHandling.CONTINUE_ON_FAILURE)
    ...
    'Memanggil DocumentMonitoring untuk dicheck apakah documentnya sudah masuk'
    WebUI.callTestCase(findTestCase('Main Flow (DEVELOPMENT MULTI DOCUMENT SIGNING)/VerifyDocumentMonitoring'), [('excelPathFESignDocument') : excelPathFESignDocument
    ...
    for (int i = 0; i < noKontrakPerDoc.size(); i++) {
        ...
        'Mengambil value dari db mengenai tipe pembayran'
        paymentType = CustomKeywords.'connection.APIFullService.getPaymentTypeMULTIDOC'(conneSign, noKontrakPerDoc[i])
        ...
        for (int d = 1; d <= 6; d++) {
            ...
            if (WebUI.verifyElementPresent(findTestObject('KotakMasuk/Sign/lbl_notrxsaldo'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
                ...
                ArrayList inquiryDB = CustomKeywords.'connection.APIFullService.gettrxSaldo'(conneSign, noKontrakPerDoc[
                    i], saldoUsed.toString(), 'Use ' + signType)

				println inquiryDB
				WebUI.delay(70)
                'check total row dengan yang tertandatangan'
                checkVerifyEqualorMatch(WebUI.verifyMatch(variableSaldoRow.size().toString(), saldoUsed.toString(), false, 
...
def checkErrorLog() {
    ...
    if (WebUI.verifyElementNotVisible(findTestObject('KotakMasuk/Sign/errorLog'), FailureHandling.OPTIONAL)) {
...
def decisionForMultiDoc(LinkedHashMap resultHashMap) {
    if (resultHashMap == [:]) {
        if (GlobalVariable.indexUsed != 0) {
            (GlobalVariable.indexUsed)--
        }
        
        return false
    } else {
        for (loopingTotalHashMap = 0; loopingTotalHashMap < resultHashMap.keySet().size(); loopingTotalHashMap++) {
            if (GlobalVariable.storeVar.getAt(GlobalVariable.storeVar.keySet()[0]) == resultHashMap.get(resultHashMap.keySet()[
                loopingTotalHashMap])) {
                if ((resultHashMap.keySet()[loopingTotalHashMap]) <= 0) {
                    if (GlobalVariable.indexUsed != 0) {
                        (GlobalVariable.indexUsed)--
                    }
