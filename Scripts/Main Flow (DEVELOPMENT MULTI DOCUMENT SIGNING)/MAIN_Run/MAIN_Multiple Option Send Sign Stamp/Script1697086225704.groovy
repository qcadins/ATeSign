...
for (GlobalVariable.NumofColm = 6
	; GlobalVariable.NumofColm <= findTestData(excelPathMain).columnNumbers; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('Option for Send Document :')) == 'API Send Document External') {
			...
			if (findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('documentid')).split(', ',-1).size() > 1) {
				signerInput = findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('$email (Send External)')).replace('\\n', '').split(
                ';', -1)
			} else {
            'ambil total signer input dari send doc external'
            signerInput = findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('$email (Send External)')).split(
                ';', -1)
			}
        ...
        'API Send Document Normal') {
			...
			documentId = findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('documentid')).split(', ', -1)

				for (loopingDocument = 0; loopingDocument < documentId.size(); loopingDocument++) { 
					signers = findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('$email (Send Normal)')).split('\n', -1)
					
					signersPerDoc = signers[loopingDocument].split(';', -1).collect { it.trim() }
        ...
        //        WebUI.callTestCase(findTestCase('Main Flow (DEVELOPMENT MULTI DOCUMENT SIGNING)/KotakMasuk'), [('excelPathFESignDocument') : excelPathMain, ('sheet') : sheet
            ...
            'Stamp Only') {
                ...
                'Yes') {
                    ...
                    'Sign Only') {
						...
						emailSignerPerDoc = CustomKeywords.'connection.SendSign.getEmailLogin'(conneSign, documentId[i]).split(';', -1)
    
						// Trim each element in the emailSignerPerDoc list
						emailSignerPerDoc = emailSignerPerDoc.collect { it.trim() }

						// Add the email signers for this document to the emailSigner list
                    ...
                    for (int i = 0; i < emailSigner.keySet().size(); i++) {
                        ...
                        if ((opsiSigning[y]) == 'API Sign Document External') {
							...
							GlobalVariable.storeVar.putAt(i, emailSigner.get(emailSigner.keySet()[i])[y])

                            'call test case api sign document external'
                            WebUI.callTestCase(findTestCase('Main Flow (DEVELOPMENT MULTI DOCUMENT SIGNING)/API Sign Document External'), [('excelPathAPISignDocument') : excelPathMain
                            ...
                            WebUI.callTestCase(findTestCase('Main Flow (DEVELOPMENT MULTI DOCUMENT SIGNING)/SignerLogin Sign'), [('excelPathFESignDocument') : excelPathMain
                                    , ('sheet') : sheet, ('opsiSigning') : opsiSigning[
