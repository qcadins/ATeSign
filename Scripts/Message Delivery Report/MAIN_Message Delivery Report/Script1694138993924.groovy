...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    ...
    0) {
        ...
        0)) {
            ...
            WebUI.callTestCase(findTestCase('Login/Login_perCase'), [('SheetName') : sheet, ('Path') : excelPathMessageDeliveryReport
...
def checkPaging(LocalDate currentDate, LocalDate firstDateOfMonth, Connection conneSign) {
    ...
    if (Integer.parseInt(totalTrxUI[0]) > 10) {
        ...
        checkVerifyPaging(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('MessageDeliveryReport/paging_Page'), 'ng-reflect-page', 
                    FailureHandling.CONTINUE_ON_FAILURE), WebUI.getAttribute(findTestObject('MessageDeliveryReport/page_Active'), 
                    'aria-label', FailureHandling.CONTINUE_ON_FAILURE).replace('page ', ''), false, FailureHandling.CONTINUE_ON_FAILURE), 
            ' button page terakhir tidak berfungsi')
