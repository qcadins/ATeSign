...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        'encrypt doc id'
        encryptdocId = CustomKeywords.'customizekeyword.ParseText.parseEncrypt'(findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, 
                rowExcel('documentid')), aesKey)

        'encrypt tenant, officecode, tenant'
        msg = encryptLink(conneSign, findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('documentid')), 
            findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('email')), aesKey)

        'HIT API Sign Document'
        responsigndocEmbed = WS.sendRequest(findTestObject('Postman/Sign Doc Embed', [('callerId') : findTestData(API_Excel_Path).getValue(
                        GlobalVariable.NumofColm, rowExcel('callerId')), ('email') : ('"' + findTestData(API_Excel_Path).getValue(
            ...
            if (status_Code == 0) {				
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
					for(timeout = 1; timeout <= 5; timeout++) {		
						...
						result = CustomKeywords.'connection.SendSign.getSignDocEmbedStoreDB'(conneSign, findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, 
								rowExcel('documentid')))
