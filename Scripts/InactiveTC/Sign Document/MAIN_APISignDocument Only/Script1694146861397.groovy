...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(API_Excel_Path).columnNumbers; (GlobalVariable.NumofColm)++) {
	if (findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		ArrayList list = [], listDocId = [], listEmail = []
		...
		emailSigner = findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('$email')).split(';', -1)
		...
		if (WS.verifyResponseStatusCode(respon_login, 200, FailureHandling.OPTIONAL) == true) {
			...
			respon_signdoc = WS.sendRequest(findTestObject('Postman/Sign Doc', [('callerId') : ('"' + findTestData(API_Excel_Path).getValue(
							GlobalVariable.NumofColm, rowExcel('callerId'))) + '"', ('email') : listEmail[0], ('documentId') : listDocId[0], ('msg') : ('"' +
						findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, rowExcel('msg'))) + '"']))
