...
if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
    ...
    if (status_Code == 0) {
        ...
        WebUI.callTestCase(findTestCase('InactiveTC/Send Document/KotakMasuk'), [('excelPathFESignDocument') : API_Excel_Path, ('jumlahsignertandatangan') : jumlahsignertandatangan
        ...
        if (findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm, 9).replace('"', '') == resultTenant) {
            ...
            WebUI.callTestCase(findTestCase('InactiveTC/Send Document/ErrorReport'), [('API_Excel_Path') : API_Excel_Path], FailureHandling.CONTINUE_ON_FAILURE)
        }
    }
} else {
    messageFailed = WS.getElementPropertyValue(respon, 'status.message', FailureHandling.OPTIONAL).toString()
