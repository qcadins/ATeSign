...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathFESignDocument).columnNumbers ; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathFESignDocument).getValue(GlobalVariable.NumofColm, 1).length() == 0) {
        ...
        'Call API Send doc'
        WebUI.callTestCase(findTestCase('InactiveTC/Beranda/ResponseAPISendDoc'), [('API_Excel_Path') : excelPathFESignDocument, ('sheet') : sheet], 
...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathFESignDocument).columnNumbers ; (GlobalVariable.NumofColm)++) {
        ...
        for (int o = 1; o <= emailSigner.size(); o++) {
            ...
            WebUI.callTestCase(findTestCase('null'), [('email') : emailSigner[(o - 1)], ('isLocalhost') : isLocalhost], FailureHandling.CONTINUE_ON_FAILURE)
		...
		if (findTestData(excelPathFESignDocument).getValue(GlobalVariable.NumofColm,86) == 'Yes') {
			'Call API Send doc'
			WebUI.callTestCase(findTestCase('InactiveTC/Flow Stamping'), [('excelPathStamping') : excelPathFESignDocument
