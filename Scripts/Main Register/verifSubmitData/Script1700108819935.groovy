...
checkVerifyEqualOrMatch(WebUI.verifyMatch(invitationDate[0], currentDate, false, FailureHandling.CONTINUE_ON_FAILURE), ' Invitation Date') //verifyListUndangan()
...
def verifyListUndangan() {
    ...
    checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getText(findTestObject('ListUndangan/table_TanggalRegistrasi')), '-', 
            false, FailureHandling.CONTINUE_ON_FAILURE), ' Tanggal Registrasi')
...
def rowExcel(String cellValue) {
    CustomKeywords.'customizekeyword.WriteExcel.getExcelRow'(GlobalVariable.DataFilePath, SheetName, cellValue)
}
