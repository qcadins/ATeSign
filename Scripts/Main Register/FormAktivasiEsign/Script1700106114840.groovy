...
if (findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('$Email')).length() > 2) {
	...
	'setting reset OTP pada DB 0'
	CustomKeywords.'connection.DataVerif.settingResetOTPNol'(conneSign, findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm,
			rowExcel('No Telepon')).replace('"', '').toUpperCase())
}
...
def inputOTP(int inputed, int delayExpiredOTP, Connection conneSign) {
	...
	if (WebUI.verifyElementPresent(findTestObject('RegisterEsign/FormAktivasiEsign/label_PopupMsg'), GlobalVariable.TimeOut,
		...
		if (findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Input Correct OTP - Aktivasi')).equalsIgnoreCase('Yes')) {
			'delay untuk menunggu OTP'
			WebUI.delay(5)
...
//			}
			...
			if (findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Setting OTP Active Duration')).length() > 0 && inputed == 0) {
