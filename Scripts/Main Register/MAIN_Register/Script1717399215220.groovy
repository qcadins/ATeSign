...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
		...
		CustomKeywords.'connection.UpdateData.updateDormantUser'(conneSign, CustomKeywords.'customizekeyword.ParseText.convertToSHA256'(findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('$NIK'))), findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Dormant User')))
...
//                }
                ...
                if (!(findTestData(excelPathRegister).getVdalue(GlobalVariable.NumofColm, rowExcel('Generate Link With')).equalsIgnoreCase(
                    'API Register'))) {
					'#Code1 ini code 44'
					/*
...
def potongSaldoSendGenInvLink(String type, HashMap<String, String> saldoBefore, Connection conneSign) {
    if (findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('$Email')).length() <= 2) {
        //perubahan v4.6
		'check jika Must use SMS message = 1'
		if ((findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Setting Must Use SMS First' + type)) ==
			'1')) {
        ...
        else if ((findTestData(excelPathRegister).getValue(GlobalVariable.NumofColm, rowExcel('Setting Must Use WA First' + type)) == 
