...
if (WebUI.verifyElementPresent(findTestObject('BuatUndangan/label_ValidationError'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
    ...
    0) {
        ...
        if (findTestData(excelPathBuatUndangan).getValue(GlobalVariable.NumofColm, rowExcel('Inquiry Invitation Action')).length() > 0) {
        	...
        	WebUI.callTestCase(findTestCase('Register_eSign/InquiryInvitation/InquiryInvitation'), [('excelPathBuatUndangan') : 'Registrasi/BuatUndangan'],
        			FailureHandling.CONTINUE_ON_FAILURE)
        }
	  } else {
...
if (findTestData(excelPathBuatUndangan).getValue(GlobalVariable.NumofColm, rowExcel('Check Inquiry Setelah Register')).equalsIgnoreCase(
    ...
    WebUI.callTestCase(findTestCase('Register_eSign/InquiryInvitation/InquiryInvitationAfterRegist'), [('excelPathBuatUndangan') : 'Registrasi/BuatUndangan'], 
        FailureHandling.CONTINUE_ON_FAILURE)
}
