...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        GlobalVariable.Tenant = CustomKeywords.'connection.APIFullService.getTenantCodeFromUser'(conneSign, findTestData(
        ...
        responOtp = WS.sendRequest(findTestObject('APIFullService/Postman/Sent Otp Signing', [('callerId') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('callerId')), ('phoneNo') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('phoneNo')), ('email') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('email')), ('refnumber') : '""', ('listDocumentId') : '""', ('vendor') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('psreCode'))]))
                ...
                'Yes') {
                    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('email')).contains('OUTLOOK.COM')) {
                        ...
                        otp = CustomKeywords.'customizekeyword.GetEmail.getEmailContent'(findTestData(excelPath).getValue(
                                GlobalVariable.NumofColm, rowExcel('email')), findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                                rowExcel('Password Outlook')), 'OTP')
                    } else {
