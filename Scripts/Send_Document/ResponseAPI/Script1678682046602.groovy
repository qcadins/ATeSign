...
for(GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(API_Excel_Path).getColumnNumbers() ; GlobalVariable.NumofColm++) 
{
	...
	List<String> ListSigner = new ArrayList<>();
	ListSigner[0] = ""
	for (int i = 1 ; i <= npwp.size() ; i ++) 
	{
		if(i == npwp.size())
		{
			list.add('{"signAction": ' + signAction[i-1] + ',"signerType": ' + signerType[i-1] + ',"signSequence":' + signSequence[i-1] + ',"alamat": ' + alamat[i-1] + ',"jenisKelamin": ' + jenisKelamin[i-1] + ',"kecamatan": ' + kecamatan[i-1] + ',"kelurahan": ' + kelurahan[i-1] + ',"kodePos": ' + kodePos[i-1] + ',"kota": ' + kota[i-1] + ',"nama": ' + nama[i-1] + ',"tlp": ' + tlp[i-1] + ',"tglLahir": ' + tglLahir[i-1] + ',"provinsi": ' + provinsi[i-1] + ',"idKtp": ' + idKtp[i-1] + ',"tmpLahir": ' + tmpLahir[i-1] + ',"email": ' + email[i-1] + ',"npwp": ' + npwp[i-1] + ',"idPhoto": ' + findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm,44) + ',"signerSelfPhoto": ' + findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm,45) + '}')
		}
		list.add('{"signAction": ' + signAction[i-1] + ',"signerType": ' + signerType[i-1] + ',"signSequence":' + signSequence[i-1] + ',"alamat": ' + alamat[i-1] + ',"jenisKelamin": ' + jenisKelamin[i-1] + ',"kecamatan": ' + kecamatan[i-1] + ',"kelurahan": ' + kelurahan[i-1] + ',"kodePos": ' + kodePos[i-1] + ',"kota": ' + kota[i-1] + ',"nama": ' + nama[i-1] + ',"tlp": ' + tlp[i-1] + ',"tglLahir": ' + tglLahir[i-1] + ',"provinsi": ' + provinsi[i-1] + ',"idKtp": ' + idKtp[i-1] + ',"tmpLahir": ' + tmpLahir[i-1] + ',"email": ' + email[i-1] + ',"npwp": ' + npwp[i-1] + ',"idPhoto": ' + findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm,44) + ',"signerSelfPhoto": ' + findTestData(API_Excel_Path).getValue(GlobalVariable.NumofColm,45) + '},')
