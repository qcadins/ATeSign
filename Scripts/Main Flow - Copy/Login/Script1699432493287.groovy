...
//belum work
    ...
    if ((checkBeforeSigning == 'Yes') || ((findTestData(excel).getValue(GlobalVariable.NumofColm, rowExcel('Option for Sign Document per Signer')).split(
        ';', -1)[GlobalVariable.opsiSigning]) == 'Sign Via Inbox')) {
        'input email'
    ...
    (GlobalVariable.FlagFailed > 0)) {
        ...
        if (errormessage != null) {
            ...
            CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, rowExcel('Status'), 
                GlobalVariable.NumofColm - 1, (((findTestData(excel).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')).replace(
                    '-', '') + ';') + '<') + errormessage) + '>')
        }
    }
}
...
def runWithEmbed(String linkUrl) {
    if ((findTestData(excel).getValue(GlobalVariable.NumofColm, rowExcel('Option for Sign Document per Signer')).split(';', 
        -1)[GlobalVariable.opsiSigning]) == 'Webview Sign') {
        ...
        ';', -1)[GlobalVariable.opsiSigning]) == 'Embed Sign') {
