        ...
        if (findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('documentid')).length() > 0) {
            ...
            if (WebUI.verifyEqual(Integer.parseInt(resultSaldoBefore.get(signType)) - GlobalVariable.eSignData.getAt('CountVerifikasiSign'), 
                ...
                WebUI.verifyEqual(Integer.parseInt(resultSaldoBefore.get('OTP')) - GlobalVariable.eSignData.getAt('CountVerifikasiOTP'), 
                    Integer.parseInt(resultSaldoAfter.get('OTP')), FailureHandling.OPTIONAL)
                ...
                if (GlobalVariable.eSignData.getAt('CountVerifikasiBiometric') > 0) {
                    ...
                    if (isSplitLivenessFc == '0') {
                        ...
                        if (WebUI.verifyEqual(Integer.parseInt(resultSaldoBefore.get('Liveness')) - GlobalVariable.eSignData.getAt(
                                'CountVerifikasiBiometric'), Integer.parseInt(resultSaldoAfter.get('Liveness')), FailureHandling.OPTIONAL) && 
                        WebUI.verifyEqual(Integer.parseInt(resultSaldoBefore.get('Face Compare')) - GlobalVariable.eSignData.getAt(
                                'CountVerifikasiBiometric'), Integer.parseInt(resultSaldoAfter.get('Face Compare')), FailureHandling.OPTIONAL)) {
                        }
                    }
                }
            } else {
            ...
            for (i = 0; i < GlobalVariable.eSignData.getAt('NoKontrakProcessed').split(';', -1).size(); i++) {
                ...
                inputFilterTrx(conneSign, currentDate, GlobalVariable.eSignData.getAt('NoKontrakProcessed').split(';', -1)[
                    i], signType)
                            ...
                            for (int u = 1; u <= variableSaldoColumn.size(); u++) {
                                ...
                                if (u == 9) {
                                    ...
                                    if ((WebUI.getText(modifyperrowpercolumn) == '1') || ((inquiryDB[(u - 1)]) == '-1')) {
                                        ...
                                        CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                                            GlobalVariable.StatusFailed, (((((findTestData(excelPathMain).getValue(GlobalVariable.NumofColm, 
                                                rowExcel('Reason Failed')) + ';') + GlobalVariable.ReasonFailedSignGagal) + 
                                            ' terlihat pada Kuantitas di Mutasi Saldo dengan nomor kontrak ') + '<') + (GlobalVariable.eSignData.getAt(
                                                'NoKontrakProcessed').split(';', -1)[i])) + '>')
                                    }
                                    ...
                                    if (u == 9) {
                                        ...
                                        if ((WebUI.getText(modifyperrowpercolumn) == '1') || ((inquiryDB[(u - 1)]) == '-1')) {
                                            ...
                                            CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, 
                                                GlobalVariable.NumofColm, GlobalVariable.StatusFailed, (((((findTestData(
                                                    excelPathMain).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + 
                                                ';') + GlobalVariable.ReasonFailedSignGagal) + ' terlihat pada Kuantitas di Mutasi Saldo dengan nomor kontrak ') + 
                                                '<') + (GlobalVariable.eSignData.getAt('NoKontrakProcessed').split(';', 
                                                    -1)[i])) + '>')
                                        }
                                    } else if (u == variableSaldoColumn.size()) {
                                        'Jika di kolom ke 10, atau di FE table saldo, check saldo dari table dengan saldo yang sekarang. Takeout dari dev karena no issue dan sudah sepakat'
