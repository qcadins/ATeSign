...
for (o = 0; o < forLoopingWithBreakAndContinue; o++) {
    ...
    result = generateEncryptMessage(conneSign, GlobalVariable.storeVar.keySet()[0], GlobalVariable.storeVar[(GlobalVariable.storeVar.keySet()[0])], tenantCode)
				...
				if (prosesMaterai != 0) {
					...
					for (i = 1; i <= 12; i++) {
						...
						if (((prosesMaterai == 51) || (prosesMaterai == 61)) && (flagErrorDMS == 0)) {
							...
							'get saldo after'
							resultSaldoAfter = WebUI.callTestCase(findTestCase('Main Flow - Copy/getSaldo'),
...
def checkSaldoWAOrSMS(Connection conneSign, String vendor) {
	...
	notifTypeDB = CustomKeywords.'connection.APIFullService.getWASMSFromNotificationTypeOTP'(conneSign, GlobalVariable.storeVar[(GlobalVariable.storeVar.keySet()[
		0])], 'OTP_SIGN_EMBED_V2', GlobalVariable.Tenant)
