...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathDownRep).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if ((findTestData(excelPathDownRep).getValue(GlobalVariable.NumofColm - 1, rowExcel('Email Login')) != findTestData(
            excelPathDownRep).getValue(GlobalVariable.NumofColm, rowExcel('Email Login'))) || (firstRun == 0)) {
            'call test case login per case'
            WebUI.callTestCase(findTestCase('Login/Login_perCase'), [('sheet') : sheet, ('Path') : excelPathDownRep, ('Email') : 'Email Login'
        ...
        for (index = 1; index <= totalData; index++) {
            modifyObjectPeriode = WebUI.modifyObjectProperty(findTestObject('DownloadReport/modifyObject'), 'xpath', 'equals', 
                ('/html/body/app-root/app-full-layout/div/div[2]/div/div[2]/app-download-report/div/div/div/div[2]/app-msx-datatable/section/ngx-datatable/div/datatable-body/datatable-selection/datatable-scroller/datatable-row-wrapper[' + 
                index) + ']/datatable-body-row/div[2]/datatable-body-cell[1]/div/p', true)

            if (findTestData(excelPathDownRep).getValue(GlobalVariable.NumofColm, rowExcel('$Report Month')).equalsIgnoreCase(
                WebUI.getText(modifyObjectPeriode))) {
                'modify button download'
                modifyObjectDownload = WebUI.modifyObjectProperty(findTestObject('DownloadReport/modifyObject'), 'xpath', 
                    'equals', ('/html/body/app-root/app-full-layout/div/div[2]/div/div[2]/app-download-report/div/div/div/div[2]/app-msx-datatable/section/ngx-datatable/div/datatable-body/datatable-selection/datatable-scroller/datatable-row-wrapper[' + 
                ...
                if (WebUI.verifyElementPresent(findTestObject('DownloadReport/errorLog'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                        GlobalVariable.StatusFailed, (findTestData(excelPathDownRep).getValue(GlobalVariable.NumofColm, 
                ...
                if ((index % 10) == 0) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                        GlobalVariable.StatusFailed, (findTestData(excelPathDownRep).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Reason Failed')) + ';') + ' Report Tidak Ditemukan')
...
def checkPaging(int resultTotalData) {
    ...
    if (Integer.parseInt(Total[0]) > 10) {
        ...
        'get total data'
        lastPage = (Double.parseDouble(WebUI.getText(findTestObject('DownloadReport/label_TotalData')).split(' ', -1)[0]) / 
        10)
        ...
        checkVerifyPaging(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('DownloadReport/activePage'), 'aria-label', 
                    FailureHandling.CONTINUE_ON_FAILURE), 'page ' + totalPage, false, FailureHandling.CONTINUE_ON_FAILURE))
