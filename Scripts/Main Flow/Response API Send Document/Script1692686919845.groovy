...
Connection conneSign = CustomKeywords.'connection.ConnectDB.connectDBeSign'()
        ...
        documentName = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 13).split(semicolon, splitnum)

        'Inisialisasi office Code berdasarkan delimiter ;'
        officeCode = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 14).split(semicolon, splitnum)

        'Inisialisasi office name berdasarkan delimiter ;'
        officeName = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 15).split(semicolon, splitnum)

        'Inisialisasi region code berdasarkan delimiter ;'
        regionCode = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 16).split(semicolon, splitnum)

        'Inisialisasi region name berdasarkan delimiter ;'
        regionName = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 17).split(semicolon, splitnum)

        'Inisialisasi business line code berdasarkan delimiter ;'
        businessLineCode = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm,inputStartRow + 18).split(semicolon, splitnum)

        'Inisialisasi business line name berdasarkan delimiter ;'
        businessLineName = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 19).split(semicolon, splitnum)

        'Inisialisasi is sequence berdasarkan delimiter ;'
        isSequence = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 20).split(semicolon, splitnum)

        'Inisialisasi psre Code berdasarkan delimiter ;'
        psreCode = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 21).split(semicolon, splitnum)

        'Inisialisasi document file berdasarkan delimiter ;'
        documentFile = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 22).split(enter, splitnum)

        'split signer untuk doc1 dan signer untuk doc2'
        signAction = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 30).split(enter, splitnum)

        signerType = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 31).split(enter, splitnum)

        tlp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 33).split(enter, splitnum)

        idKtp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 34).split(enter, splitnum)

        email = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 35).split(enter, splitnum)

        pageStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 24).split(delimiter, splitnum)

        llxStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 25).split(delimiter, splitnum)

        llyStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 26).split(delimiter, splitnum)

        urxStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 27).split(delimiter, splitnum)

        uryStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 28).split(delimiter, splitnum)
        ...
        for (int i = 0; i < documentFile.size(); i++) {
            ...
            for (int t = 0; t < signActions.size(); t++) {
                ...
                if (!(((((findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 37).length() == 0) && (findTestData(
                    excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 38).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(
                    GlobalVariable.NumofColm, inputStartRow + 39).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 
                    inputStartRow + 40).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 41).length() == 
                0))) {
                    'Split mengenai signLocation dimana berdasarkan dokumen'
                    pageSign = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 37).split(enter, splitnum)

                    llxSign = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm,inputStartRow +  38).split(enter, splitnum)

                    llySign = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 39).split(enter, splitnum)

                    urxSign = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 40).split(enter, splitnum)

                    urySign = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 41).split(enter, splitnum)

                    'split mengenai sequence Number'
                    seqNo = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 32).split(enter, splitnum)
                ...
                if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 50).length() > 0) {
                    'setting email service tenant'
                    CustomKeywords.'connection.APIFullService.settingEmailServiceVendorRegisteredUser'(conneSign, findTestData(
                            excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 50), (emails[t]).replace('"', ''))
                }
            ...
            if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 49) == 'Yes') {
            ...
            if (!(((((findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 24).length() == 0) && (findTestData(
                excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 25).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(
                GlobalVariable.NumofColm, inputStartRow + 26).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 
                inputStartRow + 27).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 28).length() == 
            0))) {
        ...
        'Jika flag tenant no'
        if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 47) == 'No') {
            'set tenant kosong'
            GlobalVariable.Tenant = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 48)
        } else if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 47) == 'Yes') {
            'Input tenant'
            GlobalVariable.Tenant = findTestData(excelPathSetting).getValue(6, 2)
        }
        
        'check if mau menggunakan api_key yang salah atau benar'
        if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 45) == 'Yes') {
            'get api key dari db'
            GlobalVariable.api_key = CustomKeywords.'connection.APIFullService.getTenantAPIKey'(conneSign, GlobalVariable.Tenant)
        } else if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 45) == 'No') {
            'get api key salah dari excel'
            GlobalVariable.api_key = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 46)
        }
		
        'Hit API'
        respon = WS.sendRequest(findTestObject('APIFullService/Postman/Send Document Signing', [('tenantCode') : findTestData(
                        excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 9), ('request') : stringRefno, ('callerId') : findTestData(
                        excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, inputStartRow + 43)]))
            ...
            if (status_Code == 0) {
               ...
               'Memasukkan documentid dan trxno ke dalam excel'
                CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, 'API Send Document', 
                    17, GlobalVariable.NumofColm - 1, documentId.toString().replace('[', '').replace(']', ''))

                CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, 'API Send Document', 
                    18, GlobalVariable.NumofColm - 1, trxno.toString().replace('[', '').replace(']', ''))
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
					...
					WebUI.callTestCase(findTestCase('APIFullService/Response API Send Document Store DB'), [('excelPathAPISendDoc') : excelPathAPISendDoc
						, ('sheet') : sheet, ('signlocStoreDB') : signlocStoreDB, ('inputStartRow') : inputStartRow], FailureHandling.CONTINUE_ON_FAILURE)
                }
            } else {
