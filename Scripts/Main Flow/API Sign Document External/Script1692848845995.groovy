...
HashMap<String, String> saldoBefore = WebUI.callTestCase(findTestCase('Main Flow/getSaldo'), 
...
if (vendor.equalsIgnoreCase('Privy') || vendor.equalsIgnoreCase('Digisign')) {
    ...
    respon_OTP = WS.sendRequest(findTestObject('APIFullService/Postman/Sent Otp Signing Non Main', [('callerId') : findTestData(
                    excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, rowExcel('callerId (Sign External)')).split(
                    ';', -1)[GlobalVariable.indexUsed], ('phoneNo') : findTestData(excelPathAPISignDocument).getValue(GlobalVariable.NumofColm, 
                    rowExcel('phoneNo (Sign External)')).split(';', -1)[GlobalVariable.indexUsed], ('email') : findTestData(
            ...
            for (int v = 1; v <= 20; v++) {
                ...
                if ((totalSignedAfter[0]) == ((totalSignedBefore[0]) + Integer.parseInt(signCount))) {
                    ...
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ...
                        responseAPIStoreDB(conneSign, ipaddress, trxNo.toString())
                    }
                    ...
                    WebUI.callTestCase(findTestCase('Main Flow (DEVELOPMENT MULTI DOCUMENT SIGNING)/VerifyDocumentMonitoring'), 
                        [('excelPathFESignDocument') : excelPathAPISignDocument, ('sheet') : sheet, ('nomorKontrak') : refNumber
                            , ('CancelDocsSign') : CancelDocsSign, ('linkDocumentMonitoring') : 'Not Used'], FailureHandling.CONTINUE_ON_FAILURE)

                    HashMap<String, String> saldoAfter = WebUI.callTestCase(findTestCase('Main Flow/getSaldo'), [('excel') : excelPathAPISignDocument
                    ...
                    for (l = 0; l < paymentTypeUsed.size(); l++) {
                        ...
                        for (String key : saldoBefore.keySet()) {
                            ...
                            if (key == (paymentTypeUsed[l])) {
                                checkVerifyEqualOrMatch(WebUI.verifyEqual(Integer.parseInt(saldoBefore.get(key)) - 1, Integer.parseInt(
                                            saldoAfter.getAt(key))), ' terhadap pemotongan saldo ' + key)
                    ...
                    nomorKontrak = CustomKeywords.'connection.APIFullService.getRefNumber'(conneSign, GlobalVariable.storeVar.keySet()[
                        0])
                    ...
                    for (String key : saldoBefore.keySet()) {
                        ...
                        if (key == signTypeUsed) {
                            checkVerifyEqualOrMatch(WebUI.verifyEqual(Integer.parseInt(saldoBefore.get(key).replace(',','')) - saldoUsed, 
                                    Integer.parseInt(saldoAfter.getAt(key).replace(',',''))), ' terhadap pemotongan saldo ' + key)
...
def responseAPIStoreDB(Connection conneSign, String ipaddress, String trxNo) {
    'get current date'
    ...
    if (trxNo != 'null') {
        ...
        'Array result. Value dari db'
        trxList = CustomKeywords.'connection.APIFullService.getTrxNoAPISign'(conneSign, (GlobalVariable.storeVar.keySet()[
            0]).toString())
