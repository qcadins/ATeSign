...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathFEPengaturanTenant).columnNumbers; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        break
    } else if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Status')).equalsIgnoreCase(
        'Unexecuted')) {
        if ((findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm - 1, rowExcel('Email Login')) != 
        findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Email Login'))) || (firstRun == 
        0)) {
            'panggil fungsi login'
            WebUI.callTestCase(findTestCase('Login/Login_perCase'), [('sheet') : sheet, ('Path') : excelPathFEPengaturanTenant
        ...
        String activationCallBackUrl, descriptionBalanceType, useWA
        ...
        if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Status')).equalsIgnoreCase(
            'Unexecuted')) {
        ...
        WebUI.verifyElementPresent(findTestObject('PengaturanTenant/label_PengaturanTenant'), GlobalVariable.TimeOut, FailureHandling.CONTINUE_ON_FAILURE)

        'Page Down'
        WebUI.sendKeys(findTestObject('PengaturanTenant/input_LabelRefNumber'), Keys.chord(Keys.PAGE_DOWN), FailureHandling.OPTIONAL)
        ...
        checkVerifyEqualorMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('PengaturanTenant/input_LabelRefNumber'), 
            ...
            WebUI.getAttribute(findTestObject('PengaturanTenant/input_LabelRefNumber'), 'value')) + ' dan ') + (resultDbPrevious[
            arrayIndex]))
        ...
        checkVerifyEqualorMatch(WebUI.verifyMatch(WebUI.getAttribute(findTestObject('PengaturanTenant/input_URLUpload'), 
                    'value'), resultDbPrevious[arrayIndex], false, FailureHandling.OPTIONAL), ((' dengan alasan tidak cocok antara ' + 
            WebUI.getAttribute(findTestObject('PengaturanTenant/input_URLUpload'), 'value')) + ' dan ') + (resultDbPrevious[
            arrayIndex]))
        ...
        for (i = 0; i < arrTipeSaldoBefore.size(); i++) {
            ...
            if (WebUI.getText(modifyObjectTipeBatasSaldo) == null) {
                ...
                CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed, 
                    (((((findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Reason Failed')) + 
                    ';') + GlobalVariable.ReasonFailedNoneUI) + 'mengenai tipe saldo ') + '<') + (arrTipeSaldoBefore[i])) + 
                    '>')
        ...
        tipeSaldo = findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Tipe Batas Saldo')).split(
            ';', -1)

        'saldo dari Tipe Saldo di split berdasarkan ;'
        saldoTipeSaldo = findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Batas Saldo')).split(
            ';', -1)
                ...
                if (WebUI.getText(modifyObjectTipeBatasSaldo) == null) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                        GlobalVariable.StatusFailed, (((((findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Reason Failed')) + ';') + GlobalVariable.ReasonFailedNoneUI) + 'mengenai tipe saldo ') + 
                        '<') + (tipeSaldo[i])) + '>')
                }
            }
        ...
        arrayEmailInput = findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Email Reminder Saldo')).split(
            ',', -1)
        ...
        if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('isStampingOtomatis')) == 
        'Yes') {
        ...
        } else if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('isStampingOtomatis')) == 
        'No') {
        ...
        if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, rowExcel('Click Coba Callback?')).equalsIgnoreCase(
            'Yes')) {
            'Klik button Coba'
        ...
        if (GlobalVariable.checkStoreDB == 'Yes') {
            ...
            WebUI.callTestCase(findTestCase('PengaturanTenant/PengaturanTenantStoreDB'), [('sheet') : sheet, ('excelPathFEPengaturanTenant') : excelPathFEPengaturanTenant
                    , ('conneSign') : conneSign, ('currentDate') : currentDate, ('tipeSaldo') : tipeSaldo, ('saldoTipeSaldo') : saldoTipeSaldo
                    , ('activationCallBackUrl') : activationCallBackUrl, ('useWA') : useWA], FailureHandling.CONTINUE_ON_FAILURE)
        }
