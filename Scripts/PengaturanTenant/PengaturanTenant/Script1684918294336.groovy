...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathFESignDocument).columnNumbers; (GlobalVariable.NumofColm)++) {
    ...
    for (i = 0; i < tipeSaldo.size(); i++) {
        ...
        descriptionBalanceType = CustomKeywords.'connection.DataVerif.getDescriptionBalanceType'(conneSign, tipeSaldo[i])
   ...
   resultDbPrevious = CustomKeywords.'connection.DataVerif.getPengaturanTenant'(conneSign, findTestData(
            'Login/Login').getValue(2, 2).toUpperCase())
   ...
   'Mengambil email dari db sebelum diedit'
	...
	'Mengambil email input dari excel dan displit '
     emaiLInput = findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 14).split(
        ',', -1)
	...
	'Jika True url Activation CallBacknya No'
    if (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 16) == 'No') {
		...
		'Jika True url Activation CallBacknya Yes, maka mengambil value dari UI'
	...
	'Jika button Simpan ada'
    if (WebUI.verifyElementPresent(findTestObject('Object Repository/PengaturanTenant/button_Simpan'), GlobalVariable.TimeOut)) {
		 'Klik button Simpan'
	...
	'verify email'
    arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 14), resultDbNew[
    ...
    arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 8), resultDbNew[
            arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))

	'verify URL upload'
    arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 9), resultDbNew[
            arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
    ...
    arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 15), resultDbNew[
            arrayIndex++], false, FailureHandling.CONTINUE_ON_FAILURE))
...
if (arrayMatch.contains(false)) {
    ...
    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed, 
        (findTestData(excelPathFEPengaturanTenant).getValue(GlobalVariable.NumofColm, 2).replace('-', '') + ';') + GlobalVariable.ReasonFailedStoredDB)
}
