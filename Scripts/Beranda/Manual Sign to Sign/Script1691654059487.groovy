...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathManualSigntoSign).columnNumbers ; (GlobalVariable.NumofColm)++) {
	if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 1).length() == 0) {
		...
		'Call API Send doc'
		WebUI.callTestCase(findTestCase('Beranda/Manual Sign'), [('excelPathManualSigntoSign') : excelPathManualSigntoSign, ('sheet') : sheet],
		...
		if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 33) == 'No') {
			'loop selanjutnya'
			continue
		}
		...
		for (int o = 1; o <= emailSigner.size(); o++) {
			...
			saldoSignBefore = checkSaldoSign(conneSign, findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 9))
			...
			if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 35) == 'Yes') {
				'Ambil data dari excel mengenai total dokumen yang ditandatangani'
				totalDocSign = findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 61).toInteger()
			} else if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 35) == 'No') {
			...
			if (!(WebUI.verifyElementPresent(findTestObject('KotakMasuk/Sign/lbl_VerifikasiPenandaTangan'), GlobalVariable.TimeOut,
				...
				'value'), findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 40), false),
				...
				if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 38) == 'OTP') {
					...
					if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 39) == 'Yes') {
					...
					if (!(WebUI.verifyElementPresent(modifyObjectlabelRequestOTP, GlobalVariable.TimeOut))) {
						...
						'Resend OTP'
						if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 45) == 'Yes') {
							'Ambil data dari excel mengenai countResend'
							countResend = findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 46).toInteger()
				...
				if (WebUI.verifyElementPresent(findTestObject('KotakMasuk/Sign/lbl_VerifikasiOTPBerhasildanMasukan'), GlobalVariable.TimeOut,
					...
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, 57, GlobalVariable.NumofColm -
						1, (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 58) + ';') + '<' + countSuccessSign + '>')
					...
					CustomKeywords.'customizekeyword.WriteExcel.writeToExcel'(GlobalVariable.DataFilePath, sheet, 58, GlobalVariable.NumofColm -
						1, (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 59) + ';') + '<' + countFailedSign + '>')

					'Jika masukan ratingnya tidak kosong'
					if (findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 48) != '') {
						...
						modifyObjectstarMasukan = WebUI.modifyObjectProperty(findTestObject('KotakMasuk/Sign/span_starMasukan'),
							'xpath', 'equals', ('//ngb-rating[@id=\'rating\']/span[' + (findTestData(excelPathManualSigntoSign).getValue(
								GlobalVariable.NumofColm, 48).toInteger() * 2)) + ']/span', true)
			...
			for (int b = 1; b <= 3; b++) {
				...
				'ambil saldo after'
				saldoSignAfter = checkSaldoSign(conneSign, findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 9))
...
if (arrayMatch.contains(false)) {
	...
	CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
		(findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 2).replace('-', '') + ';') + GlobalVariable.ReasonFailedStoredDB + ' untuk Masukan Store DB')
}
...
def masukanStoreDB(Connection conneSign, String emailSigner, ArrayList arrayMatch) {
	...
	'verify rating'
	arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 48), masukanDB[
...
def checkPopup() {
	...
	if (WebUI.verifyElementNotPresent(findTestObject('KotakMasuk/Sign/lbl_popup'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
		...
		if (!(lblpopup.contains('Kode OTP salah'))) {
			...
			CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
				(findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 2).replace('-', '') + ';') + '<' + lblpopup + '>')
...
def checkErrorLog() {
	...
	if (WebUI.verifyElementPresent(findTestObject('KotakMasuk/Sign/errorLog'), GlobalVariable.TimeOut, FailureHandling.OPTIONAL)) {
		...
		if (!(errormessage.contains('Verifikasi OTP berhasil')) && !(errormessage.contains('feedback'))) {
			...
			CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed,
				(findTestData(excelPathManualSigntoSign).getValue(GlobalVariable.NumofColm, 2).replace('-', '') + ';') + '<' + errormessage + '>')
