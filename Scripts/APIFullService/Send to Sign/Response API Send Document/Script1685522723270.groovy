...
if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 1).equalsIgnoreCase('Unexecuted')) {
    ...
    for (int i = 0; i < documentFile.size(); i++) {
                        ...
                        if (l == (pageSigns.size() - 1)) {
                            if (((pageSigns[l]) != '') || ((llxSigns[l]) != '""')) {
                                ...
                                if ((pageSigns[l]) == '') {
                                    ...
                                    l])) + ', "urx" : ') + (urxSigns[l])) + ', "ury" : ') + (urySigns[l])) + '}]') //Jika koordinat nya untuk yang pertama di signer pertama kosong
        ...
        if (!(((((findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 22).length() == 0) && (findTestData(
            excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 23).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(
            GlobalVariable.NumofColm, 24).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 
            25).length() == 0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 26).length() == 
        0))) {
    ...
    if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
        ...
        if (status_Code == 0) {
            ...
            CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, GlobalVariable.StatusFailed, 
                (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 2).replace('-', '') + semicolon) + 
                messageFailed)
        }
    } else {
        'mengambil message Failed'
