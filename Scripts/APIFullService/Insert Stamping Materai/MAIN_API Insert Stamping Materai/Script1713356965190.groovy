...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('stampPage')).length() > 0) {
            ...
            for (loopingStampPage = 0; loopingStampPage < findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel(
                    'stampPage')).toString().split(';', -1).size(); loopingStampPage++) {
                'rakit body'
                ...
                (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('notes')).toString().split(';', -1)[
                loopingStampPage])) + '"') + ', "stampLocation" : { "llx" :') + (findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                    rowExcel('llx')).toString().split(';', -1)[loopingStampPage])) + ',"lly" : ') + (findTestData(excelPath).getValue(
                    GlobalVariable.NumofColm, rowExcel('lly')).toString().split(';', -1)[loopingStampPage])) + ',"urx" : ') + 
                (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('urx')).toString().split(';', -1)[loopingStampPage])) + 
                ',"ury" : ') + (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('ury')).toString().split(
                    ';', -1)[loopingStampPage])) + '}')
        ...
        respon = WS.sendRequest(findTestObject('Postman/insertStampingMaterai', [('documentTemplateCode') : findTestData(
                        ...
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('docName')), ('docDate') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('docDate')), ('docTypeCode') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('docTypeCode')), ('docNominal') : findTestData(
                        ...
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('officeCode')), ('officeName') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('officeName')), ('regionCode') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('regionCode')), ('regionName') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('regionName')), ('businessLineCode') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('businessLineCode')), ('businessLineName') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('businessLineName')), ('taxType') : findTestData(
                        excelPath).getValue(GlobalVariable.NumofColm, rowExcel('taxType')), ('idType') : findTestData(excelPath).getValue(
                        GlobalVariable.NumofColm, rowExcel('idType')), ('idNo') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                        rowExcel('idNo')), ('taxOwedsName') : findTestData(excelPath).getValue(GlobalVariable.NumofColm, 
                        rowExcel('taxOwedsName')), ('documentFile') : bodyAPI, ('stampingLocations') : bodyStampingLocations]))
            ...
            if (statusCode == 0) {
                ...
                if (GlobalVariable.FlagFailed == 0) {
                    if (GlobalVariable.checkStoreDB == 'Yes') {
                        ArrayList resultStoreDB = CustomKeywords.'connection.Meterai.getInsertStampingMeterai'(conneSign, 
                            findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('documentNumber')))
                        ...
                        for (index = 0; index < (resultStoreDB.size() / 17); index++) {
                            ...
                            'check store db dari input'
                            arrayMatch.add(WebUI.verifyMatch((resultStoreDB[arrayIndex++]).toString().replace('.00', ''), 
                                    findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('docNominal')), 
                            ...
                            if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('regionCode')).length() > 
                            0) {
                                'check store db dari input'
                                arrayMatch.add(WebUI.verifyMatch(resultStoreDB[arrayIndex++], findTestData(excelPath).getValue(
                            ...
                            0) {
                                'check store db dari input'
                                arrayMatch.add(WebUI.verifyMatch(resultStoreDB[arrayIndex++], findTestData(excelPath).getValue(
                                            GlobalVariable.NumofColm, rowExcel('businessLineCode')), false, FailureHandling.CONTINUE_ON_FAILURE))

                                'check store db dari input'
                                arrayMatch.add(WebUI.verifyMatch(resultStoreDB[arrayIndex++], findTestData(excelPath).getValue(
                                            GlobalVariable.NumofColm, rowExcel('businessLineName')), false, FailureHandling.CONTINUE_ON_FAILURE))
                            } else {
                                arrayIndex++
                        ...
                        arrayMatch.add(WebUI.verifyEqual(resultStoreDB.size() / 17, Integer.parseInt(CustomKeywords.'connection.DocumentMonitoring.getTotalStampingandTotalMaterai'(
                                        conneSign, findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('documentNumber')))[
