...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathAPIGetSignLink).columnNumbers; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        break
    } else if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('Status')).equalsIgnoreCase('Unexecuted')) {
		
		'setting menggunakan base url yang benar atau salah'
		CustomKeywords.'connection.APIFullService.settingBaseUrl'(excelPathAPIGetSignLink, GlobalVariable.NumofColm, rowExcel('Use Correct Base Url'))
		
		'Jika flag tenant no'
		if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('use Correct Tenant Code')) == 'No') {
			'set tenant kosong'
			GlobalVariable.Tenant = findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('Wrong tenant Code'))
		} else if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('use Correct Tenant Code')) == 'Yes') {
			'Input tenant'
		...
		if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('use Correct API Key')) == 'Yes') {
			'get api key dari db'
			GlobalVariable.api_key = CustomKeywords.'connection.APIFullService.getTenantAPIKey'(conneSign, GlobalVariable.Tenant)
		} else if (findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('use Correct API Key')) == 'No') {
	    ...
	    documentId = findTestData(excelPathAPIGetSignLink).getValue(GlobalVariable.NumofColm, rowExcel('$documentId')).split(';', -1)
