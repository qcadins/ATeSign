...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        break
    } else if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('Status')).equalsIgnoreCase(
        'Unexecuted')) {
        'setting menggunakan base url yang benar atau salah'
        CustomKeywords.'connection.APIFullService.settingBaseUrl'(excelPathAPICheckStamping, GlobalVariable.NumofColm, rowExcel(
        ...
        if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('use Correct Tenant Code')) == 
        'Yes') {
            'set tenant kosong'
            GlobalVariable.Tenant = findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel(
                    'Tenant Login'))
        } else if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('use Correct Tenant Code')) == 
        'No') {
            GlobalVariable.Tenant = findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel(
                    'Wrong tenant Code'))
        }
        ...
        if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('use Correct API Key')) == 
        'Yes') {
            'get api key dari db'
            GlobalVariable.api_key = CustomKeywords.'connection.APIFullService.getTenantAPIKey'(conneSign, GlobalVariable.Tenant)
        } else if (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel('use Correct API Key')) == 
        'No') {
            'get api key salah dari excel'
            GlobalVariable.api_key = findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, rowExcel(
                    'Wrong API Key'))
        }
        ...
        if (WS.verifyResponseStatusCode(respon, 200, FailureHandling.OPTIONAL) == true) {
            ...
            if (code == 0) {
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
                    ...
                    'get data from db'
                    ArrayList result = CustomKeywords.'connection.APIFullService.getAPICheckStampingStoreDB'(conneSign, 
                    ...
                    for (index = 0; index < (result.size() / 2); index++) {
                        ...
                        for (indexResponse = 0; indexResponse < docId.size(); indexResponse++) {
                            ...
                            if (docIdDB == (docId[indexResponse])) {
                                ...
                                arrayMatch.add(WebUI.verifyEqual(result[arrayIndex++], stampingstatus[indexResponse], FailureHandling.CONTINUE_ON_FAILURE))
                    ...
                    if (arrayMatch.contains(false)) {
                        ...
                        CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                            GlobalVariable.StatusFailed, (findTestData(excelPathAPICheckStamping).getValue(GlobalVariable.NumofColm, 
