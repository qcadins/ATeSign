...
for (GlobalVariable.NumofColm = 43; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
            ...
            if (code == 0) {
                ...
                if (GlobalVariable.checkStoreDB == 'Yes') {
					...
					if (findTestData(excelPathAPIRegistrasi).getValue(GlobalVariable.NumofColm, rowExcel('psreInput')) == '""') {
						...
						arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathAPIRegistrasi).getValue(GlobalVariable.NumofColm, rowExcel('psreInput')).replace('"', '').toUpperCase(),
...
def loginAdminGetSaldo(int countCheckSaldo, Connection conneSign) {
	...
	WebUI.callTestCase(findTestCase('Login/Login_perCase'), [('SheetName') : sheet, ('Path') : excelPathAPIRegistrasi, ('Email') : 'Email Login', ('Password') : 'Password Login'
