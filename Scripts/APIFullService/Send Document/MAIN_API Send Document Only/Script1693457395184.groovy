...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= findTestData(excelPathAPISendDoc).columnNumbers; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        CustomKeywords.'connection.UpdateData.updateDBSendDocLevelNotification'(conneSign, excelPathAPISendDoc, sheet)
        ...
        respon = WS.sendRequest(findTestObject('APIFullService/Postman/Send Document Signing', [('tenantCode') : findTestData(
                        excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('$tenantCode')), ('request') : stringRefno
                    , ('callerId') : findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('callerId'))]))
        ...
        CustomKeywords.'customizekeyword.BeautifyJson.process'(responseBody, sheet, rowExcel('Respons') - 1, findTestData(
                excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('Scenario')))
            ...
            if (statusCode == 0) {
                ...
                if (documentTemplateCode.toString().replace('"', '') != responseDocTemplateCode.toString()) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                        ...
                        ' pada perbedaan document template code ')
                }
...
def getDataExcel(String semicolon, int splitnum, String delimiter, String enter) {
    ...
    qrEnable = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('QR')).split(semicolon, splitnum)
    ...
    pageStamp = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('page (stamp)')).split(delimiter, 
        splitnum)
...
def setBodyAPI(String stringRefno, String signlocStoreDB, Connection conneSign) {
    ...
    for (int i = 0; i < documentFile.size(); i++) {
        ...
        for (int t = 0; t < signActions.size(); t++) {
            ...
            if (!(((((findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('page (sign)')).length() == 
            0) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('llx (sign)')).length() == 
            0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('lly (sign)')).length() == 
            0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('urx (sign)')).length() == 
            0)) && (findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('ury (sign)')).length() == 
            0))) {
                ...
                seqNo = findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, rowExcel('SeqNo')).split(enter, 
                    splitnum)
        ...
        for (loopingEmail = 0; loopingEmail < email.size(); loopingEmail++) {
            ...
            if ((notifTypeDB == '0') || (notifTypeDB == 'Level Tenant')) {
                ...
                if (balmut.size() == 0) {
                    ...
                    CustomKeywords.'customizekeyword.WriteExcel.writeToExcelStatusReason'(sheet, GlobalVariable.NumofColm, 
                        GlobalVariable.StatusFailed, ((findTestData(excelPathAPISendDoc).getValue(GlobalVariable.NumofColm, 
                            rowExcel('Reason Failed')).replace('-', '') + ';') + 'Tidak ada transaksi yang terbentuk ketika melakukan pengiriman Informasi Signing Via ') + 
